...
function formatPriceTable(t: PriceTable): string {
  const lines = t.rows.map((r) => {
    // 🔴 D-61.C5: ป้ายชั้นโปรจริง — ตารางแจกแจงทุกจำนวน (ราคาถูกทุกแถว) แต่ "รายการโปร" มีเฉพาะแถวที่ติดป้าย
    const tag = r.isPromoTier ? " [โปรโมชั่น]" : " [ราคาตามจำนวน ไม่ใช่โปร]";
    return r.freeShip
      ? `${r.qty} ${t.unit}: สินค้า ${r.subtotal} + ส่งฟรี = รวม ${r.total} บาท${tag}`
      : `${r.qty} ${t.unit}: สินค้า ${r.subtotal} + ค่าส่ง ${r.shippingFee} = รวม ${r.total} บาท${tag}`;
  });
  return [
    `${t.name}:`,
    ...lines,
  ...
  const priceBlock =
    okTables.length > 0
      ? [
          ...
          "🔴 ป้ายท้ายแถว: [โปรโมชั่น] = โปรจริงที่ร้านตั้งไว้ — โชว์เป็น \"รายการโปรโมชั่น\" ได้เฉพาะแถวเหล่านี้ · [ราคาตามจำนวน ไม่ใช่โปร] = ตอบได้ถ้าลูกค้าถามจำนวนนั้นตรง ๆ แต่ห้ามยกไปอยู่ในรายการโปร:",
