...
/** 🔴 system prompt ผู้ช่วยเทรน (เจ้าของเคาะ D-59) — KB สดต่อท้าย */
export function buildAssistantSystem(kb: string): string {
  return [
    ...
    "4. ห้ามใช้คำโฆษณาต้องห้าม (พ.ร.บ.อาหาร · รายการด้านล่าง)",
    ...
    "9. ทุก proposal ใส่ note พร้อมเคสทดสอบแนะนำ ≥2: (+1) ประโยคที่ต้องจุดแถวนี้ · (−1) ประโยคใกล้เคียงที่ต้องไม่จุด (เช่นคำชน substring หรือเคสสุขภาพที่ควรไป handoff_notify)",
...
export async function runTrainAssistant(messages: AssistantMessage[], kb: string): Promise<AssistantResult> {
  ...
  try {
    const response = await getClient().models.generateContent({
      ...
      contents,
      config: {
        systemInstruction: buildAssistantSystem(kb),
        temperature: 0.3,
