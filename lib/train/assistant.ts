...
export function buildAssistantSystem(kb: string, excludeKeys: string[] = []): string {
  const lines = [
    ...
    "",
    'สิ่งที่ทำได้: เสนอ (ก) ร่างแถวใหม่ หรือ (ข) แก้แถวเดิม ของ 3 แท็บ: CSV_FAQ / CSV_Step / CSV_Vars — เสนอเป็น proposal เท่านั้น เจ้าของกดยืนยันเอง',
    "กติกาเหล็ก:",
    ...
    "6. ขอบเขต: เขียนได้แค่ 3 แท็บนี้ · CSV_Config = แนะนำค่าได้ แต่เขียนไม่ได้ (บอกเจ้าของไปตั้งในหน้า Config เอง) · Products/Promo = ยังไม่รองรับ (เฟสหน้า)",
