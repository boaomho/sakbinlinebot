...
describe("parse order_data — D-15: ชื่อ/ที่อยู่/เบอร์ + items[{sku,qty}] + needs_price_quote", () => {
  it("Gemini ส่ง items → parse เป็น array {sku,qty} · ไม่มี ยอด/จำนวน(ข้อความ)", async () => {
    // 🔴 พิสูจน์ parseOrderData: items แปลงเป็น {sku:string, qty:number} · needs_price_quote อ่านได้
    const full = JSON.stringify({
      reply: "ขอปลาทูคิดยอดให้สักครู่นะคะ",
      stage: "4b",
      order_data: {
        ชื่อ: "สมหญิง ใจดี",
        ที่อยู่: "123/45 ม.6 บางพลี สมุทรปราการ 10540",
        เบอร์: "0811122334",
        items: [{ sku: "NPT-10G", qty: 5 }],
      },
      needs_price_quote: true,
      payment_method: "โอน",
    });
    ...
    expect(out.orderData.items).toEqual([{ sku: "NPT-10G", qty: 5 }]);
    expect(out.needsPriceQuote).toBe(true);
    // ไม่มีช่องยอด/จำนวน(ข้อความ) อีกต่อไป
    expect((out.orderData as Record<string, unknown>).ยอด).toBeUndefined();
    vi.doUnmock("@google/genai");
    vi.resetModules();
  });

  it("items ที่ qty<=0 / sku ว่าง ถูกกรองทิ้ง (parse ไม่พังทั้งเทิร์น)", async () => {
    const full = JSON.stringify({
      reply: "x",
      stage: "2",
      order_data: { items: [{ sku: "NPT-10G", qty: 2 }, { sku: "", qty: 3 }, { sku: "NPT-20G", qty: 0 }] },
    ...
    expect(out.orderData.items).toEqual([{ sku: "NPT-10G", qty: 2 }]);
    vi.doUnmock("@google/genai");
    vi.resetModules();
  });
});
