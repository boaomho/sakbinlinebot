...
describe("parse order_data — D-20: ชื่อ/ที่อยู่/เบอร์ + items[{qty}] (AI ส่งแค่ qty)", () => {
  ...
  it("🔴 D-47 retry: blocked (no-text) ครั้งแรก → retry → ครั้งสองมี text = ผ่าน (degraded=false)", async () => {
    const full = JSON.stringify({ reply: "ได้ค่ะ", stage: "3", order_data: {}, payment_method: "โอน" });
    let calls = 0;
    vi.resetModules();
    vi.doMock("@google/genai", async () => {
      const actual = await vi.importActual<typeof import("@google/genai")>("@google/genai");
      return {
        ...
        GoogleGenAI: class {
          models = {
            generateContent: async () => {
              calls++;
              if (calls === 1) return { candidates: [], promptFeedback: { blockReason: "PROHIBITED_CONTENT" }, usageMetadata: {}, text: undefined }; // blocked
              return { candidates: [{ finishReason: "STOP" }], usageMetadata: {}, text: full }; // retry ผ่าน
            },
          };
        },
      };
    ...
    expect(calls, "เรียก 2 ครั้ง (retry 1)").toBe(2);
    expect(out.degraded, "retry ผ่าน = ไม่ degraded").toBe(false);
    expect(out.paymentMethod).toBe("โอน");
    vi.doUnmock("@google/genai");
    vi.resetModules();
  });

  it("🔴 D-47 retry: blocked 2 ครั้งติด → degraded (ไม่วนเกิน 1 retry)", async () => {
    let calls = 0;
    vi.resetModules();
    vi.doMock("@google/genai", async () => {
      const actual = await vi.importActual<typeof import("@google/genai")>("@google/genai");
    ...
    const { testConfig } = await import("../harness/fixtures");
    const out = await runSalesTurn({ config: testConfig(), configText: "", stepText: "", faqText: "", catalogText: "", objectionText: "", stateText: "", historyText: "", userMessage: "โอนเงิน", currentStage: "3" });
    expect(calls, "เรียกแค่ 2 ครั้ง (1 + retry 1 · ไม่วนต่อ)").toBe(2);
    expect(out.degraded, "บล็อกซ้ำ = degraded").toBe(true);
    vi.doUnmock("@google/genai");
    vi.resetModules();
  });
