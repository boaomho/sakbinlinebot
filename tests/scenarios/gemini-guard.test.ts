...
describe("parse order_data — D-20: ชื่อ/ที่อยู่/เบอร์ + items[{qty}] (AI ส่งแค่ qty)", () => {
  ...
  it("items ที่ qty<=0 ถูกกรองทิ้ง · sku ที่ AI ส่งมา(ถ้ามี)ถูกละทิ้ง (โค้ดใส่เอง)", async () => {
    ...
    const { testConfig } = await import("../harness/fixtures");
    const out = await runSalesTurn({
      config: testConfig(), configText: "", stepText: "", faqText: "", catalogText: "", objectionText: "", stateText: "", historyText: "",
  ...
  it("🔴 D-48 extraction: call หลัก blocked → extraction call ผ่าน → order_data/payment เข้า gate (degraded=false)", async () => {
    // call หลักถูกบล็อก · call extraction (จิ๋ว ไม่มี prompt ขาย) ผ่าน คืน order_data ล้วน
    const extractJson = JSON.stringify({ order_data: { เบอร์: "0811122334", items: [{ qty: 3 }] }, payment_method: "COD" });
    let calls = 0;
    vi.resetModules();
    vi.doMock("@google/genai", async () => {
      const actual = await vi.importActual<typeof import("@google/genai")>("@google/genai");
      return {
        ...
        GoogleGenAI: class {
          models = {
            generateContent: async () => {
              calls++;
              if (calls === 1) return { candidates: [], promptFeedback: { blockReason: "PROHIBITED_CONTENT" }, usageMetadata: {}, text: undefined }; // call หลัก blocked
              return { candidates: [{ finishReason: "STOP" }], usageMetadata: {}, text: extractJson }; // extraction ผ่าน
            },
          };
        },
      };
    ...
    const { runSalesTurn } = await vi.importActual<typeof import("@/lib/gemini")>("@/lib/gemini");
    const { testConfig } = await import("../harness/fixtures");
    const out = await runSalesTurn({ config: testConfig(), configText: "", stepText: "", faqText: "", catalogText: "", objectionText: "", stateText: "", historyText: "", userMessage: "เปลี่ยนเป็นเก็บเงินปลายทาง เบอร์ 0811122334", currentStage: "3" });
    expect(calls, "เรียก 2 ครั้ง (หลัก + extraction)").toBe(2);
    expect(out.degraded, "extraction ผ่าน = flow ต่อเนื่อง ไม่ degraded").toBe(false);
    expect(out.stage, "stage = current (route คุมประตู)").toBe("3");
    expect(out.paymentMethod, "สกัด payment ได้").toBe("COD");
    expect(out.orderData.items, "สกัด items ได้").toEqual([{ qty: 3 }]);
    expect(out.orderData["เบอร์"]).toBe("0811122334");
    vi.doUnmock("@google/genai");
    vi.resetModules();
  });

  it("🔴 D-48 extraction: call หลัก blocked + extraction ก็ blocked → degraded (ตาข่าย D-46)", async () => {
    let calls = 0;
    vi.resetModules();
    vi.doMock("@google/genai", async () => {
      const actual = await vi.importActual<typeof import("@google/genai")>("@google/genai");
      return {
        ...actual,
        GoogleGenAI: class {
          models = { generateContent: async () => { calls++; return { candidates: [], promptFeedback: { blockReason: "PROHIBITED_CONTENT" }, usageMetadata: {}, text: undefined }; } };
        },
      };
    });
    const { runSalesTurn } = await vi.importActual<typeof import("@/lib/gemini")>("@/lib/gemini");
    const { testConfig } = await import("../harness/fixtures");
    const out = await runSalesTurn({ config: testConfig(), configText: "", stepText: "", faqText: "", catalogText: "", objectionText: "", stateText: "", historyText: "", userMessage: "โอนเงิน", currentStage: "3" });
    expect(calls, "เรียกแค่ 2 ครั้ง (หลัก + extraction · ไม่วนต่อ)").toBe(2);
    expect(out.degraded, "บล็อกทั้งคู่ = degraded").toBe(true);
    vi.doUnmock("@google/genai");
    vi.resetModules();
  });
  ...
  it("🔴 D-20: ช่องว่าง/placeholder ว่าง ถูก drop (ไม่กลายเป็นข้อมูลใน gate)", async () => {
    ...
    const { runSalesTurn } = await vi.importActual<typeof import("@/lib/gemini")>("@/lib/gemini");
    const { testConfig } = await import("../harness/fixtures");
    const out = await runSalesTurn({ config: testConfig(), configText: "", stepText: "", faqText: "", catalogText: "", objectionText: "", stateText: "", historyText: "", userMessage: "x", currentStage: "2" });
