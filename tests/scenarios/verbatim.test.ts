...
describe("D-47 payment pre-check — เทิร์นเลือกวิธีจ่าย ไม่พึ่ง AI (deterministic)", () => {
  ...
  it("🔴 D-48 fix(2): มี payment=โอน อยู่แล้ว + 'เปลี่ยนเป็น COD' (AI ไม่จับ) → lock ทับเป็น COD", async () => {
    scriptGemini([
      turn({ reply: "รับค่ะ", stage: "S_OPEN", orderData: { items: [{ qty: 3 }] }, paymentMethod: "โอน" }), // เทิร์น 1: มี items + payment=โอน
      turn({ reply: "AI", stage: "S_CLOSED", paymentMethod: "" }), // เทิร์น 2: AI ไม่จับการเปลี่ยน
    ]);
    await sendText(U, "เอา 3 ถ้วย โอนค่ะ");
    expect((await readCustomer(U))?.pending_order as { การชำระเงิน?: string }, "เทิร์นแรก payment=โอน").toMatchObject({ การชำระเงิน: "โอน" });
    await sendText(U, "เปลี่ยนเป็นเก็บเงินปลายทาง เบอร์ 0811122334");
    expect((await readCustomer(U))?.pending_order as { การชำระเงิน?: string }, "pre-check detect ได้แม้มี payment อยู่ → lock COD ทับ (fix 2)").toMatchObject({ การชำระเงิน: "COD" });
  });
});

describe("D-49 · extraction-recovered + gate complete → ทวนครบจาก snapshot (ปาก-มือตรงกัน)", () => {
  it("🔴 recovered + COD ครบ + lastOrder null → override ประตู won · ทวนจาก snapshot · บอลลูน {ออเดอร์_เลขที่} ตกตามกลไก", async () => {
    seedBotLib({
      stepRows: [
        STEP_H,
        r("S1", "lead", { ตัวอย่างคำตอบ: "สวัสดีค่ะ", คิดเอง: "ปิด" }),
        r("S_DONE", "won", {
          เข้าเมื่อ: "ข้อมูลครบ",
          ตัวอย่างคำตอบ: "ทวนออเดอร์นะคะ[[แยก]]{ออเดอร์_รายการ} · ยอด {ออเดอร์_ยอด} บาท[[แยก]]ส่งที่ {ออเดอร์_ชื่อ} · {ออเดอร์_ที่อยู่} · {ออเดอร์_เบอร์}",
          ตัวอย่างประโยคปิดท้าย: "เลขที่ {ออเดอร์_เลขที่} ค่ะ",
          คิดเอง: "ปิด",
        }),
      ],
    });
    harnessOverrides.config = { raw: cfg() };
    // เทิร์น extraction: call หลัก blocked → extraction คืน order_data ครบ (recovered) · stage=S1 ชั่วคราว
    scriptGemini([
      turn({
        reply: "AI ไม่ควรถูกใช้", stage: "S1", recovered: true, paymentMethod: "COD",
        orderData: { items: [{ qty: 3 }], ชื่อ: "สมชาย ใจดี", ที่อยู่: "11 ถนนเจริญกรุง กทม 10500", เบอร์: "0811122334" },
      }),
    ]);
    await sendText(U, "เอา 3 ถ้วย เก็บปลายทาง สมชาย ใจดี 11 ถนนเจริญกรุง กทม 10500 0811122334");
    const t = customerText();
    expect(t, "#1 override → ประตู won (ทวน) ไม่ตรึง S1").toContain("ทวนออเดอร์นะคะ");
    expect(t, "รายการจาก snapshot").toContain("x3");
    expect(t, "ยอดจาก pricing (snapshot ไม่ประกอบเลขใหม่)").toContain("275");
    expect(t, "ชื่อจาก snapshot").toContain("สมชาย ใจดี");
    expect(t, "ที่อยู่จาก snapshot").toContain("ถนนเจริญกรุง");
    expect(t, "เบอร์จาก snapshot").toContain("0811122334");
    expect(t, "🔴 order_id ว่าง (cron ยังไม่แจก) → บอลลูนเลขที่ตกตาม var-guard · ไม่โชว์ค่าว่าง").not.toContain("เลขที่");
