...
/**
 * Golden scenarios (D-15 items + pricing 2-pass model)
 * - order_data ใช้ items:[{sku,qty}] · ยอด/ค่าส่ง คิดโดย lib/core/pricing (จาก fixture ชีตจริง)
 * - เทิร์นที่ลูกค้า "บอก/เปลี่ยนจำนวน" = 2-pass (กิน 2 script: pass1 "คิดยอด" + pass2 แจกแจงยอด)
 * - เทิร์นที่ให้ที่อยู่/จ่ายเงินเฉย ๆ (ไม่ส่ง items) = 1-pass
...
describe("บท 1 — ซื้อลื่น: ทัก→สั่ง(2-pass)→โอน→สลิป→ที่อยู่ → 1 แถว ยอดจาก Core", () => {
  it("เขียนออเดอร์ 1 แถว · ยอด/สินค้า+จำนวน มาจาก pricing (qty3 = 275 ส่งฟรี)", async () => {
    scriptGemini([
      turn({ reply: "สวัสดีค่ะ สนใจตัวไหนดีคะ", stage: "1" }),
      // สั่ง 3 ถ้วย → 2-pass
      turn({ reply: "ขอปลาทูคิดยอดให้สักครู่นะคะ", stage: "2", orderData: NPT(3), needsPriceQuote: true }),
      turn({ reply: "น้ำพริกปลาทู 3 ถ้วย ยอดรวม 275 บาท ส่งฟรีค่ะ[[เว้น]]สะดวกโอนหรือเก็บปลายทางดีคะ", stage: "2" }),
    ...
    expect(row["รหัสไปรษณีย์"], "H ปล่อยว่าง").toBe("");
    ...
    expect(geminiState.overflowCalls, "script ต้องพอดี (2-pass = +1 เทิร์นสั่ง)").toBe(0);
  });
});
...
describe("บท 7 — ส่งสลิปซ้ำ 2 รอบ → 1 ออเดอร์", () => {
  it("สลิปใบที่ 2 ไม่ทำให้เกิดออเดอร์ซ้ำ", async () => {
    scriptGemini([
      // สั่ง+โอน+ที่อยู่ครบในเทิร์นเดียว (มี items → 2-pass) · ยังไม่มีสลิป
      turn({ reply: "ขอเช็คยอดสักครู่นะคะ", stage: "3", paymentMethod: "โอน", orderData: { ...NPT(3), ...FULL_ADDRESS }, needsPriceQuote: true }),
      turn({ reply: "รับ 3 ถ้วย ยอดรวม 275 บาท ส่งฟรีค่ะ รอสลิปนะคะ", stage: "3", paymentMethod: "โอน" }),
...
describe("บท 8 — ส่งรูปที่ไม่ใช่สลิป → ไม่พัง ไม่สร้างออเดอร์", () => {
  it("รูป other: ไม่อัปโหลด ไม่ push แอดมิน ไม่เขียนชีต แต่ยังตอบลูกค้า (1-pass)", async () => {
    scriptGemini([
...
describe("บท 14 — 🔴 COD + ที่อยู่ก้อนเดียว (ไม่มี ต./อ./จ. นำ) → เขียนชีต", () => {
  it("ที่อยู่ก้อนดิบ + เบอร์มีขีด → ครบ → เขียน + push · ยอดจาก pricing", async () => {
    scriptGemini([
      ...
      turn({ reply: "น้ำพริกปลาทู 3 ถ้วย 275 บาท ส่งฟรีค่ะ สะดวกจ่ายแบบไหนดีคะ", stage: "2" }),
...
describe("บท 14b — 🔴 'บูม': COD qty1 · 3 field ผู้รับ ไม่มีจังหวัด/รหัสแยก", () => {
  it("3 field + items → complete · ยอด/สินค้า+จำนวน ไม่หาย (qty1 = 125)", async () => {
    scriptGemini([
      turn({ reply: "ขอคิดยอดสักครู่นะคะ", stage: "3", paymentMethod: "COD", orderData: NPT(1), needsPriceQuote: true }),
      turn({ reply: "น้ำพริกปลาทู 1 ถ้วย 95 บาท ค่าส่ง 30 รวม 125 บาท เก็บปลายทางนะคะ ขอชื่อ ที่อยู่ เบอร์ค่ะ", stage: "3", paymentMethod: "COD" }),
    ...
    expect(row["สินค้า+จำนวน"], "ต้องมี x จำนวน").toBe("น้ำพริกปลาทูฟรีซดราย x1");
...
describe("บท 16 — เบอร์รูปแบบไหนก็ได้ที่มีตัวเลข → ผ่าน", () => {
  it("COD + เบอร์บ้าน 02-1234567 (สั่ง+ที่อยู่ในข้อความเดียว) → ผ่าน", async () => {
    scriptGemini([
      turn({ reply: "ขอคิดยอดสักครู่นะคะ", stage: "3", paymentMethod: "COD", orderData: { ...NPT(3), ชื่อ: "สมชาย ใจดี", ที่อยู่: "123/45 ม.6 บางพลี สมุทรปราการ 10540", เบอร์: "02-1234567" }, needsPriceQuote: true }),
  ...
  it("โอน + เบอร์บ้าน + สลิป → ผ่าน", async () => {
    scriptGemini([
      turn({ reply: "ขอเช็คยอดสักครู่นะคะ", stage: "3", paymentMethod: "โอน", orderData: { ...NPT(3), ชื่อ: "สมชาย ใจดี", ที่อยู่: "123/45 ม.6 บางพลี สมุทรปราการ 10540", เบอร์: "021234567" }, needsPriceQuote: true }),
      turn({ reply: "รับ 3 ถ้วย 275 บาท รอสลิปนะคะ", stage: "3", paymentMethod: "โอน" }),
      turn({ reply: "ได้รับสลิปแล้วค่ะ ขอบคุณมากค่ะ", stage: "4a", paymentMethod: "โอน", imageIntent: "slip" }),
    ]);
...
describe("บท 17 — COD ได้แค่ที่อยู่ → บอทขอชื่อ+เบอร์ (ไม่เงียบ ไม่แจ้งกลุ่ม)", () => {
  it("COD ยังไม่ครบ → ไม่เขียนแถว + ไม่ push กลุ่ม", async () => {
    scriptGemini([
      turn({ reply: "ขอคิดยอดสักครู่นะคะ", stage: "3", paymentMethod: "COD", orderData: NPT(3), needsPriceQuote: true }),
      turn({ reply: "275 บาท เก็บปลายทางนะคะ ขอชื่อ ที่อยู่ เบอร์ด้วยค่ะ", stage: "3", paymentMethod: "COD" }),
    ...
    expect(lineCalls.replies.length, "2 เทิร์น = 2 reply (bubble2 ของ 2-pass ไป push ไม่ใช่ reply)").toBe(2);
    assertCentral(U);
  });
});
...
describe("บท 17b — โอน + สลิป + ยังไม่มีที่อยู่ → แจ้งกลุ่มจากสลิป ไม่ใช่ ⚠️", () => {
  it("สลิปมาก่อนที่อยู่ → admin รู้จากสลิป · ไม่มี ⚠️ ข้อมูลไม่ครบ", async () => {
    scriptGemini([
      turn({ reply: "ขอคิดยอดสักครู่นะคะ", stage: "3", paymentMethod: "โอน", orderData: NPT(3), needsPriceQuote: true }),
      turn({ reply: "275 บาท โอนได้เลยค่ะ", stage: "3", paymentMethod: "โอน" }),
...
describe("บท 18 — COD: สั่ง(ได้ชื่อ+เบอร์)→ยังไม่แจ้งกลุ่ม→ครบที่อยู่→เขียน+แจ้งกลุ่ม", () => {
  it("ระหว่างเก็บข้อมูลไม่แจ้งกลุ่ม · ครบเทิร์นไหนแจ้งเทิร์นนั้น", async () => {
    scriptGemini([
      turn({ reply: "ขอคิดยอดสักครู่นะคะ", stage: "3", paymentMethod: "COD", orderData: { ...NPT(3), ชื่อ: "สมหญิง ใจดี", เบอร์: "0811122334" }, needsPriceQuote: true }),
      turn({ reply: "น้ำพริกปลาทู 3 ถ้วย 275 บาท เก็บปลายทางนะคะ ขอที่อยู่จัดส่งด้วยค่ะ", stage: "3", paymentMethod: "COD" }),
