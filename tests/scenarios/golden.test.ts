...
describe("บท 15 — COD + เบอร์บ้าน → บอทถามเบอร์มือถือ ไม่ปิด ไม่ push", () => {
  it("02-1234567 ไม่ผ่าน COD (ต้องมือถือ 06/08/09)", async () => {
    scriptGemini([
      turn({
        reply: "ขอเบอร์มือถือ 10 หลักอีกครั้งนะคะ ขนส่งจะได้โทรหาตอนไปส่งค่ะ",
        stage: "4b",
        paymentMethod: "COD",
    ...
    expect(orderCount(), "เบอร์ไม่ใช่มือถือ = ยังไม่ปิด COD").toBe(0);
    expect(adminPushes().length, "บอทกำลังถามเบอร์เอง ยังไม่ต้องกวนแอดมิน").toBe(0);
    assertCentral(U);
  });
});

describe("บท 16 — โอน + เบอร์บ้าน → ผ่าน (เบอร์บ้านใช้ได้ถ้าไม่ใช่ COD)", () => {
  it("021234567 + สลิป → ครบ → เขียน+push", async () => {
    scriptGemini([
      turn({
        reply: "รับทราบค่ะ รอสลิปนะคะ",
        stage: "3",
    ...
    await sendImage(U);
    expect(orderCount(), "โอน + เบอร์บ้าน 10 หลัก + สลิป → ผ่าน").toBe(1);
    expect(orderRowAt(0)["เบอร์โทร"]).toBe("021234567");
    assertCentral(U);
  });
});

describe("บท 17 — ขาดที่อยู่ + สั่งแล้ว → push แอดมิน ⚠️ ห้ามเงียบ", () => {
  it("โอน+สลิป แต่ไม่มีที่อยู่ → ไม่เขียนแถว แต่ต้องแจ้งแอดมิน", async () => {
    scriptGemini([
      turn({ reply: "โอนมาที่บัญชีนี้ได้เลยค่ะ", stage: "3", paymentMethod: "โอน", orderData: { สินค้า: "น้ำพริกปลาทู", จำนวน: "3", ยอด: "285" } }),
      turn({ reply: "ได้รับสลิปแล้วค่ะ ขอชื่อ ที่อยู่ เบอร์ด้วยนะคะ", stage: "4a", paymentMethod: "โอน", imageIntent: "slip" }),
    ]);

    await sendText(U, "โอนนะคะ");
    await sendImage(U);

    expect(orderCount(), "ข้อมูลไม่ครบ = ยังไม่เขียนแถว (ยังไม่มี order_id ให้เติมทีหลัง)").toBe(0);

    const admin = JSON.stringify(adminPushes());
    expect(admin, "🔴 ลูกค้าจ่ายเงินแล้ว ห้ามเงียบ").toContain("ลูกค้าสั่งแล้วแต่ข้อมูลไม่ครบ");
    expect(admin, "ต้องบอกแอดมินว่าขาดอะไร").toContain("ที่อยู่");

    assertCentral(U);
  });

  it("push ⚠️ ครั้งเดียว ไม่ spam ทุกเทิร์น", async () => {
    scriptGemini([
      turn({ reply: "โอนมาที่บัญชีนี้ได้เลยค่ะ", stage: "3", paymentMethod: "โอน", orderData: { สินค้า: "น้ำพริกปลาทู", จำนวน: "3", ยอด: "285" } }),
      turn({ reply: "ได้รับสลิปแล้วค่ะ ขอที่อยู่ด้วยนะคะ", stage: "4a", paymentMethod: "โอน", imageIntent: "slip" }),
      turn({ reply: "ขอที่อยู่จัดส่งด้วยนะคะ", stage: "4b", paymentMethod: "โอน" }),
    ]);

    await sendText(U, "โอนนะคะ");
    await sendImage(U);
    const afterSlip = adminPushes().filter((p) => JSON.stringify(p).includes("ข้อมูลไม่ครบ")).length;
    await sendText(U, "เดี๋ยวส่งที่อยู่ให้นะ");
    const afterMore = adminPushes().filter((p) => JSON.stringify(p).includes("ข้อมูลไม่ครบ")).length;

    expect(afterSlip).toBe(1);
    expect(afterMore, "เทิร์นถัดไปต้องไม่ push ซ้ำ").toBe(1);
  });
});

describe("บท 18 — ให้ข้อมูลครบเทิร์นถัดไป → กลายเป็นครบ → เขียนแถว", () => {
  it("ไม่ครบ (push ⚠️) → เทิร์นหน้าให้ที่อยู่ → เขียนชีต", async () => {
    scriptGemini([
      turn({ reply: "เก็บปลายทางนะคะ ขอชื่อ ที่อยู่ เบอร์ด้วยค่ะ", stage: "3", paymentMethod: "COD", orderData: { สินค้า: "น้ำพริกปลาทู", จำนวน: "3", ยอด: "285", ชื่อ: "สมหญิง ใจดี", เบอร์: "0811122334" } }),
      turn({ reply: "สรุปที่อยู่จัดส่งนะคะ[[เว้น]]ของถึงภายใน 1-2 วันทำการค่ะ", stage: "4b", paymentMethod: "COD", orderData: { ที่อยู่: "123/45 ม.6 บางพลีใหญ่ บางพลี สมุทรปราการ 10540" } }),
    ]);

    await sendText(U, "เอา 3 ถ้วย เก็บปลายทาง สมหญิง ใจดี 0811122334");
    expect(orderCount(), "ยังไม่มีที่อยู่").toBe(0);
    expect(JSON.stringify(adminPushes())).toContain("ข้อมูลไม่ครบ");

    await sendText(U, "123/45 ม.6 บางพลีใหญ่ บางพลี สมุทรปราการ 10540");
    expect(orderCount(), "merge แล้วครบ → เขียนแถว").toBe(1);
