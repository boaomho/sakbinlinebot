...
import { buildStepInjection, buildFaqInjection, buildCatalogInjection, buildObjectionInjection, buildExampleInjection, readConfigDescription, resolveDestinations } from "@/lib/agent/inject";
...
describe("buildObjectionInjection — สารบัญเสมอ + เต็มเฉพาะ match + ประกอบเอง (D-27)", () => {
  const OBJ = [
    ["objection_id", "ชื่อ", "ลูกค้าพูดแบบไหนบ้าง", "ความกังวลที่แท้จริง", "หลักการตอบ", "ห้ามทำ"],
    ["OBJ_PRICE", "ราคาแพง", "แพง,แพงจัง,ราคาสูง", "กลัวไม่คุ้มเงิน", "เทียบคุณค่าต่อมื้อ ไม่ใช่ลดราคา", "ห้ามลดราคา"],
    ["OBJ_SHIP", "ค่าส่งแพง", "ค่าส่งแพง,ส่งแพง", "รู้สึกจ่ายเกิน", "ชี้โปรส่งฟรี", "ห้ามยกเว้นค่าส่งเอง"],
  ];

  it("match 'แพงจัง' → เต็ม OBJ_PRICE (ความกังวล+หลักการ+ห้ามทำ) · สารบัญมีครบทุกข้อ", () => {
...
describe("buildExampleInjection — match step/objection · เลียนน้ำเสียง ห้ามลอก (D-27)", () => {
  const EX = [
    ["step_id", "objection_id", "ตัวอย่างคำตอบที่ดี"],
    ["2", "OBJ_PRICE", "ราคานี้ตกมื้อละไม่กี่บาทเองค่ะ คุ้มมากเลย"],
    ["2", "", "ลองชิมดูก่อนได้นะคะ อร่อยแน่นอน"],
    ["3", "", "โอนแล้วส่งสลิปมาได้เลยค่ะ"],
  ];
