...
/** สร้างแถว step (ใส่เฉพาะช่องที่เทสสน · ที่เหลือ fill placeholder) · 🔴 สถานะ default "live" (v2.0 status filter) */
function step(o: Partial<Record<string, string>>): string[] {
  return STEP_HEADER.map((h) => o[h] ?? (h === "สถานะ" ? "live" : `${o.step_id ?? ""}-${h}`));
}
...
describe("buildStepInjection — region routing (D-18)", () => {
  ...
  it("🔴 D-41: handoff เต็มเฉพาะ entry-match · routing lean (ไม่มี ห้ามทำ/example/brain ใน prompt)", () => {
    ...
    expect(match, "v2.0: ไม่ยัด ห้ามทำ ใน prompt").not.toContain("ห้ามทำ");
    expect(match, "v2.0: ไม่ยัด example ใน prompt").not.toContain("ตัวอย่างคำตอบ:");
    expect(match, "handoff ไม่ยัด ความรู้สึก").not.toContain("ความรู้สึกลูกค้า");
  });
  ...
  it("🔴 D-41: fullSalesBlock = routing เท่านั้น (กรณี/เข้าเมื่อ/เก็บข้อมูล/ไปต่อ · ไม่มี example/brain)", () => {
    const rows = stepSheet();
    rows[4][STEP_HEADER.indexOf("ตัวอย่างคำตอบ (บอลลูน)")] = "แพตเทิร์นไม่ควรเข้า prompt";
    rows[4][STEP_HEADER.indexOf("กรณี")] = "เคสพิเศษ";
    rows[4][STEP_HEADER.indexOf("หลักการนำพา")] = "สมองเก่า";
    const out = buildStepInjection(rows, { quoted: false, payment: "", userMessage: "สรุปยอด" });
    expect(out, "routing: กรณี เข้า prompt").toContain("กรณี: เคสพิเศษ");
    expect(out, "v2.0: example ไม่เข้า prompt (verbatim ส่งที่ route)").not.toContain("แพตเทิร์นไม่ควรเข้า prompt");
    expect(out, "v2.0: ตัด หลักการนำพา").not.toContain("สมองเก่า");
  });
...
describe("buildStepInjection stayStage — คงประตู handoff_after_intake ข้ามเทิร์น (D-34 · additive)", () => {
  ...
  it("อยู่ H_CLAIM → คงประตูเต็ม แม้ข้อความไม่ entry-match (บอทคุย intake ต่อ)", () => {
    ...
    expect(out, "v2.0: routing block ของ intake (ไม่มีหลักการนำพาแล้ว)").toContain("[H_CLAIM] เคลม-คุยก่อน");
  });
...
describe("buildCatalogInjection — ตารางราคาสำเร็จรูปจาก calculatePrice จริง (C6 เต็มรูป · D-24)", () => {
  ...
  // 🔴 D-61.C5: ตารางแจกแจงทุกจำนวน แต่ "รายการโปร" มีเฉพาะชั้นที่ตั้งไว้จริง
  it("🔴 ป้ายแยกชั้นโปรจริง vs ราคาตามจำนวน (กันบอทโชว์โปรที่ร้านไม่ได้ตั้ง)", () => {
    const out = base();
    expect(out, "1 ถ้วย = โปรจริงในชีต").toMatch(/1 ถ้วย: .* \[โปรโมชั่น\]/);
    expect(out, "3 ถ้วย = โปรจริงในชีต").toMatch(/3 ถ้วย: .* \[โปรโมชั่น\]/);
    expect(out, "2 ถ้วย ไม่มีในชีต = ห้ามโชว์เป็นโปร").toMatch(/2 ถ้วย: .* \[ราคาตามจำนวน ไม่ใช่โปร\]/);
    expect(out, "4 ถ้วย ไม่มีในชีต").toMatch(/4 ถ้วย: .* \[ราคาตามจำนวน ไม่ใช่โปร\]/);
    expect(out, "คำอธิบายป้ายต้องอยู่ในบล็อก").toContain("โชว์เป็น \"รายการโปรโมชั่น\" ได้เฉพาะแถวเหล่านี้");
  });
});
