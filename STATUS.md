...
## 🟢 ทิศทางปัจจุบัน (D-72a · 2026-09-01) — อ่านก่อนอย่างอื่น
...
- 🔴 **ฟีเจอร์ที่ตายเงียบมาตั้งแต่ cutover (เจอใน D-68):** `handoff_after_intake` (D-34/35 intake) · `awaiting_payment`/`awaiting_address` · `post_sale` — v3 ผลิต funnel พวกนี้ไม่ได้เลย
  · แก้ทางออกไว้แล้ว: `normalizeBundle` อ่านคอลัมน์ `funnel_stage` **ถ้ามี** (ชีตจริงยังไม่มี = prod เหมือนเดิมเป๊ะ) · **เปิด intake กลับ = งาน D-70** (เจ้าของเคาะแล้ว)
- 🔴 **ปุ่มเขียนใน /train ยังปิดอยู่** — `assertWritable()` throw ก่อนแตะ Google → **แก้ชีตโดยตรงไปก่อน** · จะเปิดใน **D-72b** (เหตุจริงไม่ใช่ชื่อแท็บ — ดูข้อถัดไป)
- **D-72b (คอมมิตถัดไป)** — 🔴 **แยกเส้นทางอ่านของบอท (normalize) ออกจากเส้นทางแก้ไขของ Studio (raw)** · ไม่ใช่ "ลบ adapter" อย่างที่เคยเข้าใจ
  เหตุผลที่แท้จริงที่ปุ่มเขียนใน /train พัง = `normalizeKnowledge` **ยุบ 3 คอลัมน์เป็น `คำตอบ` ก้อนเดียว → เขียนกลับไม่ได้โดยหลักการ** (ไม่เกี่ยวกับชื่อแท็บ) · นี่คือสิ่งที่บล็อกเฟส D อยู่
  · แต่ละ logic ต้องมีเทสก่อนย้าย โดยเฉพาะ **"ว่าง=draft"** (invariant D-61.B ที่มีไว้ปิด KI-08) · ปลดล็อกปุ่มเขียนที่นั่น
...
- **เหลือทำตอนนี้:** **D-72b** (แยก raw/normalize + ปลดล็อกปุ่มเขียน) → **D-70** (เปิด intake D-34/35 กลับ) · ซ้อม **บท 2 / 4 / 5** · เฝ้าอาการ "คำถามขึ้นต้นบอลลูนปิด" (ยังไม่นิ่ง)
...
- **สิ่งที่ห้ามแตะ** (เส้นตาย): order gate · `calculatePrice` · idempotency (D-29) · last_order/S_EDIT (D-31/32) · handoff รวมศูนย์ (D-33) · เวลาไทย (D-37) · validate funnel_stage (D-38) · invariants 10 (REPO-MAP §10)
  · ⚠️ **intake (D-34-36) ยังห้ามลบ แต่ตอนนี้ไม่ทำงาน** — v3 ผลิต funnel `handoff_after_intake` ไม่ได้ · เจ้าของเคาะให้เปิดกลับใน **D-70** (D-68 ข้อ 3)
...
- 🔴 **`/train` ปุ่มเขียนชีต — ปิดตายไว้แล้ว (D-68)** · KI D-65 เดิมจดไว้ผิด (คิดว่าแค่ "ชี้ผิดไฟล์") ความจริงพัง 2 ชั้น:
  ชั้น 1 ชื่อแท็บใน `EDITABLE_TABS` เป็นชื่อ shape ภายใน ไม่มีอยู่จริงบนชีต v3 · ชั้น 2 `locateInLib` หาพิกัดจาก bundle ที่ adapter แปลงแล้ว แล้วเอาไปเขียนชีตดิบ = **เขียนผิดช่องแบบเงียบ**
  → ตอนนี้ throw ก่อนแตะ Google ทั้ง 3 ทาง (อ่าน/ซ้อม/พรีวิว ยังปลอดภัยใช้ได้)
  🔴 **แก้คำอธิบาย (D-72a):** ชั้น 1 (ชื่อแท็บ) หายแล้ว — แต่ชั้น 2 **ไม่หายเอง** เพราะเหตุจริงคือ `normalizeKnowledge` **ยุบ 3 คอลัมน์เป็นก้อนเดียว = เขียนกลับไม่ได้โดยหลักการ** → ต้องแยกเส้นทาง raw/normalize ใน **D-72b**
