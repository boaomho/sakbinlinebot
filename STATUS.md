...
- **T2-ฉ (แท็บออเดอร์ read-only · D-56) เสร็จ ✅** บน `main` — แท็บ "🧾 ออเดอร์" ในหน้า dashboard · ตารางจากชีต Orders (cache 60วิ · เรียงใหม่สุด) + สถานะ derive จากคอลัมน์จริง M/N/O/P+`shipping_notified` (`deriveOrderStatus` · cancelled ก่อนเสมอ) + headline คอขวด (งานคน=รอคอนเฟิร์ม/รอแพ็ค เน้นแดง) + TRAIN กรอง default · 🔴 read-only ล้วน (การกระทำจริงทำในชีต) · **462 passed**
- **ต่อไป T2-ค** (จัดการแถว Step/FAQ/OBJ/Vars จากเว็บ · งานเขียนชีต · reuse editor v1) → แล้ว T2-ง/จ
