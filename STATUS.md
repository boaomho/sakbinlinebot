...
## 🟢 ทิศทางปัจจุบัน (D-68 · 2026-08-28) — อ่านก่อนอย่างอื่น
...
- 🔴 **manual step ค้าง (เจ้าของทำเอง):** deploy ใหม่ขึ้น → ยืนยัน prod เขียว → **แล้วค่อยลบ `SHEET_BOTLIB_ID` + `SHEET_SCHEMA` ออกจาก Vercel** · ห้ามลบก่อน (salesCore ผูกกับ env นี้)
- 🔴 **ฟีเจอร์ที่ตายเงียบมาตั้งแต่ cutover (เจอใน D-68):** `handoff_after_intake` (D-34/35 intake) · `awaiting_payment`/`awaiting_address` · `post_sale` — v3 ผลิต funnel พวกนี้ไม่ได้เลย
  · แก้ทางออกไว้แล้ว: adapter อ่านคอลัมน์ `funnel_stage` **ถ้ามี** (ชีตจริงยังไม่มี = prod เหมือนเดิมเป๊ะ) · **เปิด intake กลับ = งาน D-70** (เจ้าของเคาะแล้ว)
- 🔴 **ปุ่มเขียนใน /train ปิดอยู่** — `writeCell`/`appendRow`/`setRowStatus` throw ก่อนแตะ Google (ชื่อแท็บ+พิกัดคอลัมน์ยังไม่ตรงชีตจริง · 2 ชั้น ดู DECISIONS D-68 §4) → **แก้ชีตโดยตรงไปก่อน** · จะเปิดใน D-69
- **เหลือทำตอนนี้:** **D-69 ถอด adapter + เปลี่ยนชื่อภายใน/ชื่อแท็บเป็นอังกฤษ** (Steps/Knowledge/Products/Promo/Vars/Config) → แล้วค่อย D-70 (intake) · ซ้อม **บท 2 / 4 / 5** · เฝ้าอาการ "คำถามขึ้นต้นบอลลูนปิด" (ยังไม่นิ่ง)
...
- 🔴 **`/train` ปุ่มเขียนชีต — ปิดตายไว้แล้ว (D-68)** · KI D-65 เดิมจดไว้ผิด (คิดว่าแค่ "ชี้ผิดไฟล์") ความจริงพัง 2 ชั้น:
  ...
  → ตอนนี้ throw ก่อนแตะ Google ทั้ง 3 ทาง (อ่าน/ซ้อม/พรีวิว ยังปลอดภัยใช้ได้) · ชั้น 2 หายเองเมื่อ D-69 ถอด adapter → เปิดเขียนที่นั่น
