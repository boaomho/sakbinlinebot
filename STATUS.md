...
## 🟢 ทิศทางปัจจุบัน (D-73 · 2026-09-02) — อ่านก่อนอย่างอื่น
...
- **D-73 เสร็จ ✅ (ฝั่งโค้ด)** — เปิด intake D-34/35 กลับ: precedence 4 ข้อบังคับในโค้ด+เทสครบ · handoff จาก intake
  แนบ "📋 ข้อมูลที่เก็บได้" (บทสนทนาช่วงเก็บข้อมูล · เทิร์นรูป = "[ลูกค้าส่งรูปมา]") · keyword handoff reset ตัวนับ
  🔴 **รอเจ้าของเพิ่มแถว `H_CLAIM`/`H_BULK` ในชีต Steps (draft ก่อน → ซ้อม → live)** — ตารางเป๊ะในรายงานแชท D-73
  · rollback = toggle แถวเป็น draft (พิสูจน์ด้วยเทสแล้ว ไม่ต้อง deploy) · ⚠️ known risk "ห้ามรับปากผลลัพธ์" คุมด้วย
  prompt เท่านั้น — เจอบอทรับปากแม้ครั้งเดียว → ยกระดับเป็น guard ทันที (DECISIONS D-73)
