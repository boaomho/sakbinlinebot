...
## 🟢 ทิศทางปัจจุบัน (D-77 · 2026-09-03) — อ่านก่อนอย่างอื่น
...
- **D-75 เสร็จ ✅ (เฟส D)** — ผู้ช่วยเทรนสัมภาษณ์: ปุ่มเลือกงาน (เพิ่ม Knowledge/Steps/Vars · แก้แถวเดิมพร้อมค้นหา id)
  → สัมภาษณ์ตามสคริปต์ผูก schema จริง → ใบ proposal ผ่านด่านตรวจ deterministic (คอลัมน์ตรงชีต/keyword ชน+substring/
  H1 เกณฑ์เดียวกับ lint/เรื่องซ้ำชี้ id) → **📝 เติมฟอร์มเดิม** (เพิ่มแถว D-72b / ✎ D-74) เจ้าของแก้แล้วบันทึกเอง
  🔴 ประตูเขียนบานเดียว — ตัด confirmProp (เขียนตรงจากการ์ด) ทิ้ง · โมเดลผู้ช่วย = ตัวเดียวกับบอท (ชีตตั้ง) ·
  ทุก call ลง `ai_usage` call_kind "assistant" · ⚠️ รอเจ้าของลองใช้จริง 1 รอบ (สัมภาษณ์→ฟอร์ม→บันทึก→▶ ซ้อม)
- **D-76 เสร็จ ✅** — Messenger: แอดมินพิมพ์แทรก → บอทหยุด (เทียบเท่า LINE) · 🔴 **พบว่ากลไกมีตั้งแต่ M-2 แต่ตายเงียบ**:
  echo event ของ Meta กลับด้าน (sender=เพจ · recipient=ลูกค้า) → โค้ดเดิมปิดบอทให้ id ผี `fb:<page>:<page>`
  ลูกค้าจริงไม่เคยถูกปิด · เทสเดิมผ่านเพราะ fixture ใส่ sender=PSID ซึ่งไม่ใช่รูปจริง (ความมั่นใจปลอม D-68 ข้อ 2)
  · ตัวแยก echo บอท/คน เพิ่มลายเซ็น `metadata="sakbin-bot"` = deterministic 100% (app_id เป็นชั้นรอง)
  · คืนสิทธิ์บอทใช้เส้น `processMessage` เดียวกับ LINE ไม่มีกติกาใหม่ — เพิ่มแค่เทส e2e ยืนยัน
- **D-70 เสร็จ ✅** — แท็บ **💸 ต้นทุนบอท** ใน dashboard ร้านจริง: ลูกค้า/เทิร์น/regen% /token/เงินแยก 4 call_kind
  (ผู้ช่วยเทรนแยกเห็น) · ต่อลูกค้า · ต่อออเดอร์ (แหล่งเดียวกับการ์ดยอดขาย) · กราฟ 7/30 วัน · เลือกวันย้อนหลังได้
  🔴 เงิน = **ประมาณ** (คำนวณซ้ำจาก token ด้วยตารางราคาแหล่งเดียวใน gemini.ts) · โมเดลนอกตารางราคา = **นับแยก
  "ไม่ทราบราคา N แถว" ห้ามเป็น 0** · ขอบวัน = เวลาไทย (SQL shift +7 ก่อนตัดวัน — ai_usage เก็บ UTC)
  · ⚠️ ตัวเลขจะมีให้ดูจริงเมื่อ prod เดินไปสักพัก (ตาราง ai_usage เริ่มเก็บตั้งแต่ D-69)
- **D-77 เสร็จ ✅** — บอทแก้ จำนวน/รายการ/ยกเลิก ออเดอร์ **COD ก่อนมี tracking** ได้เอง (แพทเทิร์น D-31 เดิม):
  ราคาใหม่จาก computeQuote (ตารางโปรจริง) · ยกเลิก = พยาน 2 ชั้น (tag AI + คำจริงในข้อความ · Config `คำ_ยกเลิกออเดอร์`)
  · 🔴 **นโยบายถาวร "เงินเคลื่อนแล้ว = คน"**: โอนแล้ว/มี tracking/กำกวม → handoff เสมอ (money guard อ่านจากแถวชีตจริง)
  · เขียนพลาด = ห้ามพูดเรียบร้อย + ปิดบอท · snapshot ในแชท sync ตามชีตทุกเส้น (ปิดบั๊กเก่า) · คีย์ใหม่
  `ข้อความ_แจ้งแอดมิน_แก้ออเดอร์` (default ไม่มี "รบกวน")
- **เหลือทำตอนนี้:** **D-71** (explicit caching ถ้า `cached_tokens` = 0 — ดูได้จากหน้า D-70 แล้ว) · ซ้อม **บท 2 / 4 / 5** · เฝ้าอาการ "คำถามขึ้นต้นบอลลูนปิด" (ยังไม่นิ่ง)
