...
## 8. Orders sheet (A–Z) — header-driven

**A–R คงเดิมทุกช่อง** (ทีมแพ็คไม่ต้องเรียนรู้ใหม่) · **เพิ่ม S–Z**

| col | header | ใครเขียน | หมายเหตุ |
|---|---|---|---|
| A | `ลำดับ (แจกตอนคอนเฟิร์ม)` | cron | **ไม่ใช่ key** |
| B | `วันที่` | บอท | ISO8601 + tz |
| C–J | ชื่อไลน์ · ชื่อ-นามสกุล · เบอร์ · ที่อยู่ · ตำบล · อำเภอ · จังหวัด · ไปรษณีย์ | บอท | |
| K | `สินค้า+จำนวน` | บอท | ข้อความคนอ่าน: `น้ำพริกปลาทู x3 \| น้ำปลาร้า x2` |
| L | `ยอดเงิน` | บอท | ยอดที่จ่ายจริง (รวมค่าส่ง) |
| M–N | การชำระเงิน · รูปSlip | บอท | |
| O–P | คอนเฟิร์ม · ยกเลิก | คน | ติ๊กทั้งคู่ = ยกเลิก |
| Q–R | ส่งออเดอร์แล้ว · เลขTracking | cron / คน | |
| **S** | `order_id` | บอท | `SKB-YYYYMMDD-xxxxxx` · **idempotency key** |
| **T** | `line_user_id` | บอท | join key กับ Neon |
| **U** | `items_json` | บอท | หลาย SKU ใน 1 แถว |
| **V** | `ค่าส่ง` | บอท | `subtotal = L − V` |
| **W** | `source_channel` | บอท | `line` \| `salepage` \| `shopee` … |
| **X** | `ref_code` | บอท | ไม่มี = ว่าง (**ห้ามเดา**) |
| **Y** | `ยอดในสลิป` | **คน** | ไม่ตรง L = ห้ามคอนเฟิร์ม |
| **Z** | `bot_version` | บอท | `prompt_v1.5/lib_v1.5` |
...
- assert: `sum(line_total) + ค่าส่ง(V) == ยอดเงิน(L)` ไม่ตรง = **ไม่เขียน + log**
