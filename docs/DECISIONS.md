...
- **เฟส ง · mobile polish** (commit นี้ · CSS/layout ล้วน): แชทเต็มจอ · bottom sheet (backdrop/drag-to-close/dvh+safe-area กันคีย์บอร์ดบัง) · ปุ่ม/ฟอนต์นิ้วโป้ง · **bug fix เฟส ค:** เปิด editor → fetch ข้อความดิบสดจากชีต (mode:diff) แทนค่าที่ติดมากับเทิร์นเก่า (กัน stale หลังเขียน) + badge "ชีตถูกแก้แล้วหลังเทิร์นนี้"
