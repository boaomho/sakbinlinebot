...
### D-77 · ขยายการแก้ออเดอร์ของบอท — จำนวน/รายการ/ยกเลิก บน COD ก่อนส่ง (1 commit)
นโยบายเจ้าของ (จดเป็นเส้นถาวร): **"เงินเคลื่อนแล้ว = คน"** — บอทแก้เอง/ยกเลิกเองได้เฉพาะ
**COD ที่ยังไม่มี tracking** · โอนแล้ว (เพิ่ม/ลด/ยกเลิก = เก็บเพิ่มหรือคืนเงิน) · มี tracking ·
เคสกำกวม → handoff เสมอ · แพทเทิร์น = เส้นแก้ที่อยู่ D-31 ที่พิสูจน์แล้ว (สรุป → ยืนยัน → เขียน → 🔔
→ พูด "เรียบร้อย" ได้เฉพาะหลังเขียนสำเร็จ)

#### สิ่งที่พบตอนสำรวจ (รายงานแล้วก่อนลงมือ)
- เส้นแก้ items **มีอยู่แล้ว** ใน editHandled (D-31) แต่ **ไม่มี money guard เลย** — ออเดอร์โอนก็แก้ยอดได้เงียบ ๆ
- state ไม่ sync (ข้อ 7): แก้สำเร็จแล้ว `lastOrder` snapshot ไม่อัปเดต → `{ออเดอร์_ยอด}` พูดค่าเก่า (บั๊กเดิม
  ของเส้นที่อยู่ด้วย) · AI พูด "แก้ให้แล้ว" ก่อนรู้ผลเขียน — เขียนพลาดคำโกหกหลุด

#### ที่ทำ
- **money guard ใน `updateOrderRow`** — อ่านจาก **แถวชีตจริง** (ความจริง ไม่ใช่ state): การแก้ที่แตะ
  `สินค้า+จำนวน/ยอดเงิน/ค่าส่ง/items_json` ต้องเป็น COD (K ตรงตัว) + `เลขTracking` ว่าง + `ยกเลิก`≠TRUE
  → ไม่ผ่านคืน `money_locked` + เหตุผล → handler override reply อบอุ่นไม่รับปากผล + 🔔 + ปิดบอท
  · แก้ชื่อ/ที่อยู่/เบอร์ = พฤติกรรมเดิมเป๊ะ (มีเทสยืนยัน — แก้เบอร์บนออเดอร์โอนยังได้)
- **ราคาใหม่จาก `computeQuote`** (ตารางโปรจริง — เครื่องเดียวกับตอนสั่ง) · เทสยืนยัน 3→5 ถ้วย เขียน 440
  จากโปร P5 ไม่ใช่เลขของโมเดล · order_id เดิม แถวเดิม (diff ต่อเซลล์ · ค่าเท่าเดิมไม่เขียนซ้ำ)
- **`cancelOrderRow`** — ติ๊ก `ยกเลิก`=TRUE + ประวัติ Y + นับ Z · **ห้ามลบแถว** · guard เดียวกับแก้เงิน
- 🔴 **ยกเลิก = พยาน 2 ชั้น (เจ้าของเคาะเพิ่ม)** — N=TRUE คือ action ทำลายล้างเดียวในชุดนี้:
  ชั้น 1 AI ส่ง `tags_add: ["ยกเลิกออเดอร์"]` (อ่าน tag ดิบ ไม่ผูกสวิตช์ tagging) ·
  ชั้น 2 โค้ดเช็คเองว่าข้อความลูกค้า 1-2 เทิร์นล่าสุดมีคำตระกูลยกเลิกจริง (`hasCancelEvidence` ·
  Config `คำ_ยกเลิกออเดอร์` default ยกเลิก,ไม่เอาแล้ว,ไม่สั่งแล้ว,ขอเลิก) · tag มาแต่ไม่มีหลักฐาน →
  **ห้ามแตะชีต · fallback = handoff + 🔔 ไม่ใช่เงียบ** (หลัก C5: สัญญาณต้องมาจากข้อมูล ไม่ใช่โมเดลฝ่ายเดียว)
- **🔔 เส้นเงิน** = `✏️💰 ยอดเก็บเงิน COD เปลี่ยน` เดิม→ใหม่ + เตือน "ถ้าแพ็ค/จ่าหน้าแล้วต้องแก้" · ไม่ปิดบอท
  · ยกเลิก = `🚫` + "แถวยังอยู่ ไม่ได้ลบ" · ไม่ปิดบอท
- **เขียนพลาด** (try/catch ใหม่ — เดิมไม่มี) → override reply "บันทึกไม่สำเร็จ...ส่งทีมแอดมิน" (ห้ามมีคำ
  เรียบร้อย) + 🔔 + ปิดบอท · **override กลไกเดียว**: `editReplyOverride` ทับ outReply หลัง resolve
- **sync state (ข้อ 7)**: แก้สำเร็จ → `setLastOrder` merge ค่าใหม่ (รวมเส้นผู้รับเดิม — ปิดบั๊กเก่า) ·
  ยกเลิกสำเร็จ → `clearLastOrder` (snapshot+lock ล้าง · แถวชีต/orders_written/ประวัติ คงอยู่)
- **สอน AI ผ่าน `lastOrderLine`** (user content ใน handler — ไม่แตะ system-v3.ts): สรุปยอดใหม่จากตาราง →
  ขอยืนยัน 1 ครั้ง → ค่อยส่ง flag · ยกเลิกยืนยัน 1 ครั้ง → tag · ห้ามพูดสำเร็จเด็ดขาด ("กำลังดำเนินการให้")
- **เก็บตก**: 🔔 เส้นแก้เดิมมี "รบกวน" 2 จุด → แก้ default + คีย์ Config ใหม่ `ข้อความ_แจ้งแอดมิน_แก้ออเดอร์`

#### ก่อนเขียนชีต + precedence
ยังสั่งไม่จบ = บล็อกนี้ไม่ทำงานเลย (`hasWrittenOrder` gate — ปลอดภัยโดยโครงสร้าง · มีเทส tag โผล่กลาง
การสั่ง → order flow เดิม) · คำ_handoff pre-check อยู่ก่อนถึงบล็อกนี้ → ชนะเสมอ (มีเทส)

#### เกณฑ์ผ่าน
เทสใหม่ 16 เคส (unit guard/cancel/evidence + scenario เต็มสาย 8) · golden ชั้น D diff ว่าง ·
npm test 667 passed · ไม่แตะ system-v3.ts/guards เดิม/ORDERS_HEADER/สูตร/Apps Script

### D-70 · หน้าต้นทุนบอทใน dashboard — อ่านจาก `ai_usage` (1 commit)
เป้า: เจ้าของเปิดดูแล้วตอบได้ใน 10 วินาที ว่า "วันนี้บอทคุยกี่คน ใช้เงินกี่บาท ต่อออเดอร์คุ้มไหม"
แท็บใหม่ **💸 ต้นทุนบอท** ใน dashboard ร้านจริง (read-only · ไม่มี cron · ไม่แตะเส้นบอท)

#### ตัวเลข (default วันนี้ · เลือกวันย้อนหลังได้ · กราฟ 7/30 วัน)
ลูกค้าที่บอทคุย (distinct user_id · ตัด channel `train`) · เทิร์น (`main`) + regen เป็น % ของ main ·
token in/out · เงินรวม + แยก `main`/`regen`/`extraction`/`assistant` (ต้นทุนผู้ช่วยเทรนแยกเห็น) ·
ต่อลูกค้า 1 คน · ต่อ 1 ออเดอร์ (ออเดอร์จาก `orders_written` + ยอดในชีต ข้ามที่ยกเลิก = **แหล่งเดียวกับการ์ดยอดขาย**)

#### กติกาความจริงของตัวเลข (ทั้งสามข้อมีเทส)
- 🔴 **เงิน = ประมาณ** — `ai_usage` เก็บแค่ token/model → คำนวณซ้ำด้วย `estimateCostThb` ที่ export จาก
  `gemini.ts` (**ตารางราคาแหล่งเดียวกับ log ต่อ call** ไม่ลอก) · ป้าย "ประมาณ — ตัวเลขจริงดู cost log ของ Google"
  ติดหัวแท็บ + ทุกการ์ดเงิน (`COST_APPROX_NOTE`)
- 🔴 **unknown-model-price นับแยก** — โมเดลนอกตารางราคา → `rowCostThb` = null → เข้า `unknownCalls`
  โชว์เป็นแถบ "ไม่ทราบราคา N แถว" · **ไม่นับเป็น 0 ไม่ถัวเฉลี่ย** (ไม่งั้นต้นทุนดูถูกกว่าจริงแบบเงียบ ๆ)
  · จำนวน "ครั้ง" ยังนับครบทุกแถว — ขาดแค่ยอดเงิน
- 🔴 **ขอบวัน = เวลาไทย** — `ai_usage.at` เก็บ UTC → SQL `to_char(at + interval '7 hours', 'YYYY-MM-DD')`
  ก่อน GROUP BY · เทสตรงจุด: เทิร์นเวลา 00:30 ไทย (17:30Z ของ "เมื่อวาน" UTC) ต้องเข้ายอด **วันนี้**
  (ถ้าตัดด้วย UTC ยอดจะเพี้ยน 7 ชม.)

#### จำนวน query คงที่ (ไม่ N+1)
ต่อการโหลด 1 ครั้ง: `aiUsageDaily` 1 · `aiUsageDailyCustomers` 1 · `wonOrdersSince` 1 · `orderAmountMap` (ชีต · cache 60 วิ) 1
🔴 distinct ลูกค้าต้องแยก query — รวมข้าม model/call_kind ไม่ได้ (บวกกันแล้วนับซ้ำ) · ทุก query เป็น aggregate
ฝั่ง SQL ไม่ดึงแถวดิบ · การพับเป็นซีรีส์/แยก kind ทำใน `summarizeAiUsage` (pure · เทสได้โดยไม่มี DB)

#### เกณฑ์ผ่าน
เทสใหม่ 14 เคส (สูตรเงินต่อ token · แยก kind · unknown แยกนับ · ขอบวันไทย · เติมวันว่างในกราฟ · เลือกวันย้อนหลัง)
· golden ชั้น D diff ว่าง · npm test 651 passed · ไม่แตะ เส้นบอท/guards/ชีต
