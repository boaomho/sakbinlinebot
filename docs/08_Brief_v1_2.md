...
1. **`last_slip_pathname` ใน customers (deterministic)** — ลูกค้ามักส่งสลิปเทิร์นหนึ่งแล้วพิมพ์ที่อยู่เทิร์นถัดไป ต้องจำ pathname ข้ามเทิร์นถึงเขียนคอลัมน์ N ได้ถูกแถว · เขียนด้วย `GREATEST(last_slip_pathname, new)` เก็บใบล่าสุด (timestamp ในชื่อไฟล์สูงกว่า) กันส่งสลิปหลายใบพร้อมกันแล้ว invocation แข่งเขียนทับกันมั่ว
