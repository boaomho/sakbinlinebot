...
24. บันทึก Neon: `addMessage(user/assistant)`, `updateCustomerAfterTurn(stage, tags)`, `logFunnelEvent`, ถ้า resume แล้ว `clearResumeNotice`
25. `replyMessages(replyToken, finalReply, quotaSaver)` → fail → `pushMessages`
26. **ถ้ามีรูป → `handleImageIntent`** (คืน `slipThisTurn`) · slip=อัปโหลด+push ADMIN 💰+รูป · damage=หลักฐาน+push+humanMode · other=ไม่ทำ · imageFallback→บังคับ slip
27. **`order_edit_request && has_written_order`** → push ADMIN `✏️` + `setHumanMode true` (`editHandled=true`, ห้ามแก้แถวเอง)
28. **orders+memory && !editHandled → `runOrderGate`** (merge pending_order → COMPLETE เขียนชีต+push / ไม่ครบ ติดแท็กรอ + เคส paid-no-address)
29. `handoff switch && AI handoff && !damageHandled && !editHandled` → `pushHandoffNotice` + `setHumanMode true`
...
customers(
  ...
  last_slip_pathname TEXT,           -- pathname สลิปล่าสุดที่ยังไม่ผูกออเดอร์ (GREATEST=ใบล่าสุด)
  ...
  pending_order JSONB,               -- order_data สะสมข้ามเทิร์น (merge) จนกว่า gate จะครบ
  has_written_order BOOL DEFAULT false,     -- เขียนออเดอร์ลงชีตแล้ว (ไม่ reset · ไว้บอก AI → order_edit_request)
  paid_no_address_notified BOOL DEFAULT false, -- flag แจ้งแอดมิน "โอนแล้วยังไม่มีที่อยู่" ครั้งเดียว
...
## 9. ระบบออเดอร์ + สลิป (โมเดล "สถานะลื่นไหล" — code-gate)

**หลักการ:** ออเดอร์คือสถานะที่สะสมข้ามเทิร์น ไม่ใช่ฟอร์ม · **โค้ดตัดสินจาก `pending_order` ที่มีจริงเท่านั้น** · ชีตรับเฉพาะออเดอร์ "สมบูรณ์" (ไม่มีแถวครึ่ง ๆ)

**รูปทุกชนิด (`handleImageMessage`):** ดาวน์โหลด → ส่งให้ Gemini พร้อมบริบทครบ **ไม่อัปโหลดก่อน** · `handleImageIntent` คืน pathname สลิป (ถ้า slip):
- **slip** → `uploadSlip` (private, `slips/{YYYY-MM}/{userId}_{ms}.jpg`) + เก็บ `last_slip_pathname` (GREATEST) + **push จุดที่ 1 → `ADMIN_GROUP_ID`**: `💰 มีลูกค้าส่งสลิปมาค่ะ\n{image_note}\nLineOA:{ชื่อ}` + รูป signed URL · คืน pathname
- **damage** → อัปโหลดหลักฐาน + push `ADMIN_GROUP_ID` `⚠️ ลูกค้าแจ้งปัญหา/เคลม` + รูป + `setHumanMode(true)`
- **other** → ไม่ทำ (รวมรูปที่มีที่อยู่ — AI อ่านข้อความในรูปลง order_data เอง)

**order gate (`runOrderGate` · เมื่อสวิตช์ orders+memory):**
1. **merge** `order_data` (ฟิลด์ไม่ว่าง) + `payment_method` (ถ้าไม่ว่าง) → `customers.pending_order`
2. `addressComplete(pending)` = ชื่อ + ที่อยู่+ตำบล+อำเภอ+จังหวัด+รหัสไปรษณีย์ + เบอร์ 10 หลัก (`sanitizePhone`)
3. `slipPresent` = สลิปเทิร์นนี้ **หรือ** `last_slip_pathname` เดิม
4. **COMPLETE** เมื่อ `COD && addr` หรือ `โอน && addr && slipPresent`:
   - `appendOrderRow` เขียนแถว (COD: N ว่าง · โอน: N=pathname) — ถ้า throw = ไม่ล้าง ไม่ push (retry เทิร์นหน้า)
   - สำเร็จ → `clearPendingOrderAndSlip` + `reconcileWaitTags(null)` + `setHasWrittenOrder` + reset paid-flag
   - **push จุดที่ 2 → `ADMIN_GROUP_ID`**: COD `📦 ออเดอร์ใหม่ (COD)\n...` · โอน `💰 ออเดอร์ใหม่ (โอน)\n...` + รูปสลิป
5. **ไม่ครบ = สภาพปกติ** (ไม่เขียนชีต ไม่รบกวนแอดมิน) → ติดแท็กรอ:
   - โอน+addr+ไม่มีสลิป → `รอโอน` · COD+ไม่มี addr → `รอที่อยู่` · payment ยังไม่ตัดสิน → ไม่มีแท็ก
   - **เคสเดียวที่แจ้งแอดมิน:** โอน+มีสลิป+ไม่มี addr → `รอที่อยู่` + push `💰 ลูกค้าโอนแล้ว แต่ยังไม่ได้ที่อยู่` **ครั้งเดียว** (flag `paid_no_address_notified`)

**ขอแก้ออเดอร์ที่เขียนแล้ว (`order_edit_request=true` + `has_written_order`):** push `ADMIN_GROUP_ID` `✏️ ลูกค้าขอแก้ออเดอร์ที่บันทึกแล้ว: {reason}` + `setHumanMode(true)` · **ห้ามเขียน/แก้แถวอัตโนมัติ** · ข้าม gate เทิร์นนั้น

**สรุปจุด push ทั้งระบบ (มีแค่ 3 + 2 พิเศษ):** (1) ส่งสลิป→ADMIN 💰+รูป ทันที · (2) ออเดอร์ครบ→ADMIN 📦/💰 · (3) ติ๊กคอนเฟิร์ม→cron→ORDER_GROUP เลข+ที่อยู่ · พิเศษ: โอนแล้วยังไม่มีที่อยู่ (💰 ครั้งเดียว) · ขอแก้ออเดอร์ (✏️+handoff)

**`appendOrderRow` เขียนแถวลงชีต Orders (คอลัมน์ A-R):**
```
...
12. **ออเดอร์ = สถานะลื่นไหล code-gate (ไม่มี order_action)** — ยกเลิก `order_action` (AI signal) เปลี่ยนเป็นโค้ดตัดสินจาก `pending_order` (merge สะสม field-level: ส่งมา=ทับ ไม่ส่ง=คงเดิม กันข้อมูลหายเมื่อลูกค้าให้ทีละเทิร์น) · `payment_method` ประเมินใหม่ทุกเทิร์น (เปลี่ยน COD↔โอนได้) · gate: COD ครบเมื่อที่อยู่ครบ / โอน ครบเมื่อที่อยู่ครบ+สลิป · ครบ→เขียนชีตทีเดียว (ไม่มีแถวครึ่ง) · ไม่ครบ=ปกติ ติดแท็กรอ (โค้ดจัดการ ไม่ให้ AI ใส่) · แจ้งแอดมินแค่ "โอนแล้วยังไม่มีที่อยู่" (flag ครั้งเดียว) · ขอแก้ออเดอร์ที่เขียนแล้ว→`order_edit_request`→handoff (has_written_order ไม่ reset · ห้ามแก้แถวอัตโนมัติ)
