...
**`handleImageMessage`:**
15. ถ้าสวิตช์ orders เปิด: `downloadMessageContent` (จาก LINE) → `uploadSlip` (Blob private) → เตรียม base64 ให้ Gemini · placeholder text = "[ลูกค้าส่งรูปสลิป/หลักฐานการโอนมา]" (ถ้า orders ปิด = "[ลูกค้าส่งรูปมา]") → `processMessage(placeholder, image, slipPathname)`

**`processMessage` (แกนหลัก):**
16. ถ้ามี memory: `ensureCustomer` (สร้าง/อัปเดต last_seen) → ถ้า `display_name` ยังว่าง เรียก `getProfileName` เก็บลง Neon (ไว้ค้นในคำสั่งแอดมิน)
17. **เช็ค human_mode:** ถ้า `human_mode=true` → ดู "แชทเงียบ" = `now - last_seen(เดิม)` ≥ `adminSilenceReturnMinutes` นาที? → ถ้าใช่ ปลดล็อก (`setHumanMode false`) และดำเนินต่อ · ถ้าไม่ใช่ → บันทึกข้อความลูกค้า แล้ว **return เงียบ** (แอดมินดูแลอยู่ บอทไม่ตอบ)
18. เก็บ `slipPathname` ล่าสุดลง Neon (เผื่อเก็บที่อยู่เทิร์นถัดไป)
...
21. `runSalesTurn(Gemini)` หุ้ม `withTimeout(8000ms)` → เกิน = fallback DEFAULT_REPLY
22. gate ด้วยสวิตช์: `effectiveTagsAdd` (ถ้าติดแท็กปิด = []), `effectiveHandoff` (ถ้า handoff ปิด = false), `effectiveOrderAction` (ถ้า orders ปิด = none)
...
26. ถ้า orders เปิด และ order_action ≠ none → `handleOrderAction` (สลิป/COD ยิง **ADMIN_GROUP_ID** · address เขียนชีต)
27. ถ้า `effectiveHandoff=true` → `pushHandoffNotice` + `setHumanMode true`
...
| `maxOutputTokens` | `maxOutputTokens` | `max_output_tokens` | 1024 (Math.max 1024) | **hard** (Gemini) |
...
**รับสลิป (ลูกค้าส่งรูป, `handleImageMessage` → Gemini `order_action="slip_received"`):**
1. `downloadMessageContent(messageId)` ดึงรูปจาก LINE
2. `uploadSlip(userId, buffer)` → Blob **private** store (`BLOB_SLIPS_TOKEN`), path = `slips/{YYYY-MM}/{userId}_{timestamp}.jpg`, `addRandomSuffix:false`
3. เก็บ `slipPathname` ลง `customers.last_slip_pathname` (ข้ามเทิร์น)
4. ส่ง base64 รูปให้ Gemini อ่านบริบท (ไม่ได้ให้ AI อ่านตัวเลขในสลิป — โค้ดเป็นคนจัดการ)
5. `handleOrderAction("slip_received")`: `getSlipSignedUrl(pathname, slipUrlExpiryDays)` สร้าง signed GET URL (issueSignedToken → presignUrl) → **push เข้า `ADMIN_GROUP_ID`** (กลุ่มเช็คยอด) ข้อความ `💰 มีลูกค้าส่งสลิปมาค่ะ\n{สินค้า x จำนวน}\n\nLineOA: {ชื่อ}` + รูป signed URL

**COD (`order_action="cod_confirmed"`):** push **`ADMIN_GROUP_ID`** ข้อความ `📦 ขอ CF COD ค่ะ\n{สินค้า x จำนวน}\n\nLineOA: {ชื่อ}`
...
- **MAX_TOKENS:** finishReason MAX_TOKENS → DEFAULT_REPLY (กันตอบครึ่งประโยค) · `maxOutputTokens` บังคับ ≥1024
...
1. **`last_slip_pathname` ใน customers** — ลูกค้ามักส่งสลิปเทิร์นหนึ่งแล้วพิมพ์ที่อยู่เทิร์นถัดไป ต้องจำ pathname ข้ามเทิร์นถึงเขียนคอลัมน์ N ได้ถูกแถว
