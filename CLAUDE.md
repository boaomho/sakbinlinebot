...
- 🔴 **DNA การขายอยู่คนละที่ตามโหมด — `lib/gemini.ts` เลือก prompt ตาม `isSchemaV3()`**
  (D-65: ข้อความเดิม "กฎเหล็ก 10 ข้อ ในบล็อก `<ขั้นตอนการตอบ>`" เป็นของ v1 — **ไม่มีบล็อกนี้ในโค้ดทั้งสองโหมดแล้ว** ห้ามนับเลขข้อจากเอกสาร ให้ดูหัวข้อบล็อกในโค้ด)
  - **v2 (`prompt/system.ts` · D-44 · prod ตอนนี้):** AI **ไม่ได้เขียนข้อความถึงลูกค้า** — คำที่ลูกค้าเห็นมาจาก pattern ในชีต (verbatim · D-40) · prompt เหลือ 4 งาน: เลือกประตู · จำแนก objection · สกัด order_data · ตัดสิน handoff → **DNA การขายอยู่ในชีต ไม่ใช่ใน prompt**
  - **v3 (`prompt/system-v3.ts` · D-61):** บอทเขียนเอง — DNA การขายกระจายอยู่ในบล็อก `<หมวกนักขาย 3 ใบ>` · `<3C>` · `<กลไกตอบแทรก-แล้วพากลับ>` · `<เส้นทางการขาย 4 ประตู>` · `<เคสสุขภาพ/แพ้อาหาร>` · `<งานที่ต้องทำต่อเทิร์น>` · `<รูปแบบคำตอบ>` (ไม่ได้เรียงเป็นข้อ 1-10)
  - **สิ่งที่จริงทั้งสองโหมด (โค้ดบังคับ ไม่พึ่ง prompt):** จบทุกเทิร์นด้วยทางเลือกที่พาไปประตูถัดไป · ห้าม "รับมั้ยคะ" · ห้าม "รบกวน" · FAQ แล้ววกกลับ funnel · เข้าประตูไหนก็ได้ · **ปิดท้ายด้วยข้อความเสมอ ห้ามจบด้วยรูป** (`enforceTextLast` + log) · ไม่รู้/ไม่มีข้อมูล = บอกตรงๆ + handoff
...
🔴 **เส้นที่ไม่ขยับคือ "ห้ามคำรับรอง" ไม่ใช่ "ห้ามบอทตอบ"** — วิธีบรรลุต่างกันตามโหมด:

| | **v2 (prod ตอนนี้)** | **v3 (D-61)** |
|---|---|---|
| เจอ H1 | **handoff ทันทีเสมอ** (keyword pre-check + AI) — บอทไม่คุยต่อ | **บอทคุยต่อได้** — ถามกลับเพื่อเข้าใจ + ให้**ข้อเท็จจริงตามฉลาก** (ส่วนประกอบ/สารก่อภูมิแพ้/ไลน์ผลิตเท่าที่มี) แล้วให้ลูกค้าตัดสินใจเอง |
| คำรับรอง | ไม่เกิด (ไม่ได้ตอบ) | 🔴 **ห้ามทุกรูปประโยค** — ทานได้/กินได้/ปลอดภัย/ไม่เป็นไร/หายห่วง/ไม่มีปัญหา · ห้ามแม้เชิงระวัง ("เพื่อความปลอดภัย…") และห้ามรูปมีเงื่อนไข ("ถ้าไม่แพ้ก็ทานได้") |
...
| แอดมินรู้ไหม | รู้ (handoff) | **รู้เสมอ** — ธง 🔔 `pushHealthNotifyV3` ยิงทุกเทิร์นที่ติดธงสุขภาพ แม้ AI ล้ม/ส่งไม่ถึง (D-61.C2) · dedup ที่ `__HEALTH_NOTIFY__` |

- ไม่มีข้อมูล/ไม่แน่ใจ (เช่น cross-contact) = **บอกตรงๆ ว่าไม่ทราบ + ให้แอดมินเช็คให้** ทั้งสองโหมด
- 🔴 **ห้ามใส่ "หลักการตอบ" เรื่องสุขภาพ/แพ้อาหารลงคลังความรู้ในชีต** (v2 = `CSV_Objections` · v3 = แท็บ **ความรู้** ที่ adapter map เป็น FAQ+OBJ) — ใส่ "หลักการ" แล้วบอทจะประกอบคำตอบเอง และวันหนึ่งจะประกอบผิด · **ใส่ได้เฉพาะข้อเท็จจริงตามฉลาก** (v3 อ่านสารก่อภูมิแพ้จากแท็บสินค้าเข้า prompt โดยตรงอยู่แล้ว)
- H2 ต่อรอง/ขอส่วนลด · H3 เคลม/ของเสีย · H4 ร้องเรียน/ขู่ฟ้อง → handoff ทั้งสองโหมด (แต่ H1 อันตรายสุด)
...
- ❌ ทำให้ "สิ่งที่จริงทั้งสองโหมด" (ดูหัวข้อ หัวใจของบอท) หลุด — โดยเฉพาะ "ปิดท้ายด้วยข้อความ" (ถ้าบอลลูนสุดท้ายเป็นรูป โค้ดต้องสลับ + log) · 🔴 นับเลขข้อจากโค้ด ห้ามจำ "9/10 ข้อ" จากเอกสาร
...
- ❌ **เขียนโค้ด/เอกสารรองรับ v2 เพิ่ม — v2 frozen ตั้งแต่ D-66 (2026-08-20)** · งานใหม่เขียนแบบ **v3-only** ห้ามออกแบบรองรับสองโหมด · โค้ด v2 ที่มีอยู่คงไว้เป็นตาข่าย rollback เท่านั้น (เงื่อนไขถอด → DECISIONS D-66)
...
**spec/อื่น:** `docs/FOLLOW-SPEC.md` `[UNBUILT]` (Follow engine อนาคต) · `prompt/system.ts` (v2) + `prompt/system-v3.ts` (v3) = โค้ด system prompt จริง **ทั้งคู่** (ไม่ใช่ doc · ห้ามแตะตอนงานเอกสาร · `gemini.ts` เลือกด้วย `isSchemaV3()`) · `DIAG-LOG.md` = log วินิจฉัยชั่วคราว (ลบได้เมื่อปิดบั๊ก)
