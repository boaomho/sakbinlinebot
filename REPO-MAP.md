...
lib/
  ...
  sheets/         # อ่าน Google Sheets (header-driven)
    ...
    loader.ts       loadBotLibrary() — batchGet 6 แท็บ 1 call + cache 60วิ
  ...
  train/          # T-STUDIO ห้องซ้อมเทรน (/train · เฟส ก+ข)
    ...
    lint.ts         เฟส ข/T2-ค: lintPattern (unknown-var/**var-collision/var-empty (D-67 · warn)**/claims/price/bubbles/health-h1 · reuse guard production) · lintHealthH1 (trigger-aware · แถวเกี่ยวสุขภาพ→คำตอบต้อง handoff)
    write.ts        เฟส ค/T2-ค: diffCell/listTabRows/suggestNextKey (อ่าน) · writeCell/appendRow/setRowStatus — 🔴 **D-68 ปิดทางเขียนชั่วคราว** `assertWritable()` throw ก่อนแตะ Google (ชื่อแท็บ+พิกัดคอลัมน์ยังไม่ตรงชีต v3 · 2 ชั้น ดู DECISIONS D-68 §4 · เปิดใน D-69) · guard เดิม (conflict/lint/funnel/dup/draft-forced/hard guard ห้าม Orders) ยังทำงานครบก่อนถึงจุด throw
...
**lib/sheets/loader.ts** — `loadBotLibrary(): Promise<BotLibrary|null>` batchGet **6 แท็บ** (`SHEET_TABS` จาก normalize-bundle.ts) + cache → `normalizeBundle` · 🔴 **D-72a: ชื่อแท็บบนชีต = คีย์ใน `BotLibrary` เป๊ะ** (Steps/Knowledge/Products/Promo/Vars/Config) — `BOTLIB_TABS` (7) = `SHEET_TABS` (6) + `Follow` (dormant · คืน [] เสมอ) · อ่าน `SHEET_BOTLIB_ID` เสมอ
**lib/sheets/normalize-bundle.ts** — 🔴 **D-72a เปลี่ยนชื่อจาก `adapter-v3.ts` (ไม่ใช่ shim ที่ลบทิ้งได้)** — `SHEET_TABS`/`SheetTab` · `normalizeBundle(rawByTab) → BotLibrary` · `lintV3Bundle` · เหลือ **logic จริง 4 ก้อน** ที่ลบไม่ได้: (1) สถานะ "ว่าง=draft" (2) คิด funnel_stage (FIXED_FUNNEL · หรืออ่านคอลัมน์ถ้ามี) (3) ก้อนคำตอบ Knowledge = ความกังวล+ข้อเท็จจริง+แนวตอบ **3 คอลัมน์→ช่องเดียว (เขียนกลับไม่ได้ → D-72b)** (4) lowercase สถานะ Products/Promo
...
- 🔴 **KI-08 (D-68)** ปุ่มเขียนใน `/train` **ปิดตายอยู่** — ชั้น 1 ชื่อแท็บใน `EDITABLE_TABS` เป็นชื่อ shape ภายใน ไม่มีบนชีต v3 · ชั้น 2 `locateInLib` หาพิกัดจาก bundle ที่ adapter แปลงแล้วไปเขียนชีตดิบ = **เขียนผิดช่องเงียบ** → ชั้น 2 หายเองเมื่อ D-69 ถอด adapter · แก้ชีตโดยตรงไปก่อน
