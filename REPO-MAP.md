...
**lib/sheets/loader.ts** — `loadBotLibrary(): Promise<BotLibrary|null>` batchGet **6 แท็บ v3** (`V3_SHEET_TABS`) + cache → `adaptV3Bundle` คืน shape ภายใน · `BOTLIB_TABS` = **คีย์ของ shape ภายใน (8 ช่อง)** ไม่ใช่ชื่อแท็บบนชีต · 🔴 **D-68 ไม่มี dispatch v2/v3 แล้ว** (อ่าน `SHEET_BOTLIB_V3_ID` เสมอ)
...
| `SHEET_BOTLIB_V3_ID` | loader.ts (batchGet ทุกแท็บ v3) + train/write.ts — **คุม `salesCore`+`follow`** (D-68 ย้ายการ์ดมาจาก `SHEET_BOTLIB_ID`) |
...
| ~~`SHEET_SCHEMA`~~ ~~`SHEET_BOTLIB_ID`~~ | 🔴 **ลบแล้ว D-68** — โค้ดไม่อ่าน · เจ้าของลบออกจาก Vercel **หลัง** deploy ใหม่เขียว |
