...
**lib/orders.ts** — `appendOrderRow(input+orderId+lineUserId)` (**KI-06 fix: เขียน R=line_user_id ทุกแถว**) · `readAllOrderRows()` (private · header-driven) → `listPendingOrders()` (M+!O+!cancel) / **D-50** `listOrdersToNotifyShipping()` (O=TRUE+P≠ว่าง+!cancel) · `markOrderSent(row, num)` · **D-31** `updateOrderRow(orderId, changes, now) → {status:updated/confirmed/not_found/no_change, changed[]}` (แก้แถวเดิมโดย order_id · M=TRUE→confirmed · Y ประวัติ/Z นับ) · `ORDERS_HEADER` (26 คอล A–Z: +Y แก้ไขล่าสุด +Z แก้ไขกี่ครั้ง · header-driven)
