...
**lib/orders.ts** — `appendOrderRow(input+orderId+lineUserId)` (**KI-06 fix: เขียน R=line_user_id ทุกแถว**) · `readAllOrderRows()` (private · header-driven) → **D-50/D-64** `listOrdersToNotifyShipping()` (**A(ลำดับ)≠ว่าง** + P≠ว่าง + !cancel · 🔴 ห้ามพึ่ง O) · ~~`listPendingOrders`/`markOrderSent`~~ **ลบ D-64** · **D-31** `updateOrderRow(orderId, changes, now) → {status:updated/confirmed/not_found/no_change, changed[]}` (แก้แถวเดิมโดย order_id · M=TRUE→confirmed · Y ประวัติ/Z นับ) · `ORDERS_HEADER` (26 ชื่อที่โค้ด **ต้องใช้** · header-driven) · 🔴 **D-64: ชีตจริงมี 27 คอลัมน์** — แทรก Q "กล่องส่งออเดอร์" (สูตรให้คน copy) → order_id=R … แก้ไขกี่ครั้ง=AA · **ห้ามใส่ชื่อนี้ใน ORDERS_HEADER** (จะกลายเป็น required)
