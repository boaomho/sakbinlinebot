...
lib/
  ...
  sheets/         # อ่าน Google Sheets (header-driven)
    ...
    loader.ts       loadBotLibrary() (เส้นบอท·normalize) + 🔴 D-72b loadRawSheets() (เส้น Studio·แถวดิบ) — batchGet 6 แท็บ 1 call + cache 60วิ ร่วมกัน
    normalize-bundle.ts  🔴 D-72a (ชื่อเดิม adapter-v3.ts) — normalizeBundle(): จุด normalize ความหมายจุดเดียว
  ...
  line.ts         LINE Messaging API — reply/push/download/parseReply · **D-61.C6** `splitClosingQuestion` (ดันคำถามพาไปต่อเป็นบอลลูนเดี่ยว · ตัดที่ขอบบรรทัด · ชน cap 5 → รวมสองบอลลูนแรก) + `isClosingQuestion`/`CLOSING_QUESTION_RE` (**แหล่งเดียว** · golden ชั้น G import ใช้ร่วม) · **D-67** รูป URL ไม่ใช่ http(s) → log event `image-dropped` ที่ `parseReplyIntoMessages` (ครั้งเดียวต่อ delivery · studio อ่านไปโชว์ขีดฆ่า)
  orders.ts       Google Sheet Orders — append/list (header-driven) · 🔴 D-64 ไม่มี mark แล้ว (ไม่เขียน A/O)
...
app/api/
  ...
  cron/orders/route.ts    🔴 D-64: เหลืองานเดียว = D-50 แจ้งพัสดุ (แจกเลข/ยิงกลุ่มแพ็ค ย้ายไป Apps Script + คน copy)
...
lib/train/dashboard.ts # T2-ก/ฉ pure: channelOf · deriveStatus(🟢🟡🔴✅⚪) · formatOrderSummary (ห้าม JSON ดิบ) · deriveOrderStatus+ORDER_STATUS_META (🔴 D-64: อิง A/N/P ไม่พึ่ง O · ตัด awaiting_number · cancelled ก่อนเสมอ)
# 🔴 คอลัมน์สถานะ (D-57.1): ชีต v3 ทุกแท็บใช้ "สถานะ"(ไทย) — แต่ `statusColumnIndex(header)` ที่ inject.ts ยังรับ "status"(อังกฤษ) ด้วย (จุดเดียว · loader+write.ts ใช้ร่วม · ห้าม hardcode ชื่อเดียว)
...
# 🔴 D-70 api/dashboard/cost (แท็บ 💸 ต้นทุนบอท · read-only) — aiUsageDaily + aiUsageDailyCustomers (SQL shift +7 ก่อนตัดวัน) + wonOrdersSince/orderAmountMap (ออเดอร์ = แหล่งเดียวกับการ์ดยอดขาย) → lib/train/ai-cost.ts `summarizeAiUsage` (pure) · เงิน = ประมาณ คำนวณซ้ำด้วย `estimateCostThb` (export จาก gemini.ts · ตารางราคาแหล่งเดียว) · โมเดลนอกตาราง = `unknownCalls` นับแยก ห้ามเป็น 0
...
tests/
  ...
  # 🔴 D-68 botlib-fixture.ts: `TAB` (ชื่อแท็บที่เดียวทั้ง repo · D-69 แก้บรรทัดเดียว) + `v3StepRows()`/`v3KnowRows()`
  #   seed "ชีตดิบ v3" → ผ่าน loader+adapter จริง · ห้าม seed shape ปลายทางตรง ๆ (= ความมั่นใจปลอม)
  #   `geminiState.lastInput` = prompt ที่โมเดลได้รับจริง — v3 เรียบเรียงสด สิ่งที่ชีตคุมคือ prompt ไม่ใช่ข้อความลูกค้า
...
**lib/sheets/loader.ts** — `loadBotLibrary(): Promise<BotLibrary|null>` batchGet **6 แท็บ** (`SHEET_TABS` จาก normalize-bundle.ts) + cache → `normalizeBundle` · 🔴 **D-72b:** `loadRawSheets(): Promise<RawSheets|null>` = **แถวดิบตามชีตเป๊ะ** จาก batchGet เดียวกัน (cache entry เดียว `{raw,bundle}` — เส้น Studio/ปุ่มเขียนเท่านั้น ห้ามเข้า prompt) · 🔴 **D-72a: ชื่อแท็บบนชีต = คีย์ใน `BotLibrary` เป๊ะ** (Steps/Knowledge/Products/Promo/Vars/Config) — `BOTLIB_TABS` (7) = `SHEET_TABS` (6) + `Follow` (dormant · คืน [] เสมอ) · อ่าน `SHEET_BOTLIB_ID` เสมอ
**lib/sheets/normalize-bundle.ts** — 🔴 **D-72a เปลี่ยนชื่อจาก `adapter-v3.ts` (ไม่ใช่ shim ที่ลบทิ้งได้)** — `SHEET_TABS`/`SheetTab` · `normalizeBundle(rawByTab) → BotLibrary` · `lintV3Bundle` · 🔴 **D-72b exports ใหม่:** `composeKnowledgeAnswer(concern,fact,guide)` (ก้อนคำตอบ Knowledge — แหล่งเดียว normalize+preview) · `isLiveStatus(raw)` (สถานะแถวดิบ "ว่าง=draft" — ห้ามใช้ isActiveStatus กับแถวดิบ botlib) · `isHandoffFlag(raw)` · เหลือ **logic จริง 4 ก้อน** ที่ลบไม่ได้: (1) สถานะ "ว่าง=draft" (2) คิด funnel_stage (FIXED_FUNNEL · หรืออ่านคอลัมน์ถ้ามี) (3) ก้อนคำตอบ Knowledge = ความกังวล+ข้อเท็จจริง+แนวตอบ **3 คอลัมน์→ช่องเดียว (เขียนกลับไม่ได้ → D-72b)** (4) lowercase สถานะ Products/Promo
...
**lib/agent/inject.ts** — `buildStepInjection(rows, {quoted,payment,userMessage,signals?})` **region routing (D-19)** · **D-32** signals(order_editable/order_confirmed_locked)→ประตูที่ "เข้าเมื่อ" มี token ยัดเต็ม · **D-33** `funnelStageOf(rows, stepId)` (โค้ดการันตี handoff เมื่อ funnel_stage=handoff) · **D-34** `stepNameOf(rows, stepId)` + `stayStage`(คงประตู handoff_after_intake ข้ามเทิร์น · additive) + validStages รับ handoff_after_intake · **D-38** `validateStepFunnelStages(rows)→BadFunnelStage[]` + `VALID_FUNNEL_STAGES`(9) (loader เรียกตอนโหลด · console.error · diag `/api/diag/steps`): โค้ดตัดสิน funnel จาก pending (quoted=มี items → S4 · ไม่มี → S1-S3) ไม่พึ่ง AI stage · สารบัญทุกประตู + เต็ม cap 4 (priority: match วิธีจ่าย>ปลายทาง>entry-match>proximity) · handoff+crossover(ประตูข้าม ไม่มีใครชี้มา) เต็มเฉพาะ entry-match ไม่นับ cap · `buildCatalogInjection(products, promo, {config,payment,now,methodDescription})` **D-24 C6 เต็ม**: ยัด "ตารางราคาสำเร็จรูป" (สินค้า+ทุกจำนวน 1..เพดาน จาก `buildPriceTable`→calculatePrice ตัวเดียวกับ gate · payment ตาม pending · calc ล้ม→ไม่ยัดตาราง+handoff) แทนตารางโปรดิบ · **D-61.C5** ทุกแถวติดป้าย `[โปรโมชั่น]` (=`PriceTableRow.isPromoTier` จาก `isExactTier`) หรือ `[ราคาตามจำนวน ไม่ใช่โปร]` — prompt โชว์ "รายการโปร" ได้เฉพาะแถวติดป้ายแรก (กันบอทเสนอขั้นราคาที่ร้านไม่ได้ตั้ง · ราคาถูกทุกแถว guard ไม่เกี่ยว) · `readConfigDescription(configRows, key)` อ่านคอลัมน์คำอธิบาย (วิธีคิดจากชีต) · ~~**D-27** `buildObjectionInjection`~~ (🔴 **ลบ D-72a** — v3 ยุบ Objections เข้า Knowledge ตั้งแต่ D-61.B · คืน text ว่างมาตลอด) · `buildFaqInjection` · `resolveDestinations` · **🔴 v2.0 (D-41):** `STEP_COLS`/`FAQ` required เฉพาะที่โค้ดอ่าน · `fullSalesBlock`/`leanHandoffBlock` = routing เท่านั้น (ตัด example/brain จาก prompt · pattern ส่ง verbatim ที่ route) · `isActiveStatus` กรองแถว draft/ปิดทุกแท็บ · **ลบ `buildExampleInjection`/CSV_Examples** (เลิก "เลียนโทน") · **D-42:** `buildFaqInjection→{text,verbatim}` (action=answer→verbatim answer · handoff→null) + `stepClosing(rows,stepId)` · route precedence **handoff>FAQ>step** (isHandoffTurn ตัด FAQ · 🔴 D-72a ตัด objection ออกจากลำดับ) · **D-47** `detectPaymentChoice(msg)`/`isPaymentChoiceOnly(msg)`/`resolvePaymentStep(rows,payment)` (payment pre-check เส้นทางเงิน · ข้าม AI เทิร์นเลือกจ่าย) · `redactFinancial(text,accounts,phones)→{text,count}` (ปิดบังเลขบัญชี/เบอร์ใน input โมเดลเท่านั้น · D-48 คืน count) · **P2 ชั้น③** `parseThinkMode(raw)→"เปิด"|"ปิด"` (**D-40: ว่าง/ไม่มีคอลัมน์/ไม่รู้จัก=ปิด verbatim · เฉพาะ เปิด/on/1/ใช่=เปิด override**) + `stepVerbatim(rows, stepId)→{mode,pattern}|null` — โหมดปิด route ส่ง `pattern` เป๊ะ (verbatim) แทน reply ที่ AI แต่ง · **D-39B2** `pattern` = `joinVerbatimParts(ตัวอย่างคำตอบ, ปิดท้าย)` = 2 ช่องคั่น `[[แยก]]` อัตโนมัติ (ข้ามช่องว่าง · ปิดท้าย=บอลลูนสุดท้าย) · คอลัมน์ `คิดเอง` = optional (ไม่มี=เปิด · ไม่ regression)
**lib/orders.ts** — `appendOrderRow(input+orderId+lineUserId)` (**KI-06 fix: เขียน R=line_user_id ทุกแถว**) · `readAllOrderRows()` (private · header-driven) → **D-50/D-64** `listOrdersToNotifyShipping()` (**A(ลำดับ)≠ว่าง** + P≠ว่าง + !cancel · 🔴 ห้ามพึ่ง O) · ~~`listPendingOrders`/`markOrderSent`~~ **ลบ D-64** · **D-31/D-77** `updateOrderRow(orderId, changes, now) → {status:updated/confirmed/not_found/no_change/money_locked, changed[], row}` (แก้แถวเดิมโดย order_id · M=TRUE→confirmed · 🔴 D-77 money guard: แก้คอลัมน์เงินต้อง COD+ไม่มีTracking+ไม่ยกเลิก ไม่งั้น money_locked · Y ประวัติ/Z นับ) · **D-77** `cancelOrderRow(orderId, now)` (ติ๊ก N=TRUE + Y/Z · guard เดียวกัน · ห้ามลบแถว) · `ORDERS_HEADER` (26 ชื่อที่โค้ด **ต้องใช้** · header-driven) · 🔴 **D-64: ชีตจริงมี 27 คอลัมน์** — แทรก Q "กล่องส่งออเดอร์" (สูตรให้คน copy) → order_id=R … แก้ไขกี่ครั้ง=AA · **ห้ามใส่ชื่อนี้ใน ORDERS_HEADER** (จะกลายเป็น required)
**lib/gemini.ts** — 🔴 **D-69** `resolveThinkingConfig(model, raw)` (pure · เลือก thinkingLevel/thinkingBudget ตามตระกูล) · `resolveGeminiTimeouts(mainMs, debounceMs, maxDurationMs)` (pure · clamp ตามงบ) · `PRICE_PER_1M_USD` + log `scope:"ai-usage"` (latency/tokens/cached/cost — **ราคาประมาณเท่านั้น ตัวจริงดู cost log ของ Google**) · เขียน `ai_usage` ลง Neon ทุกการเรียก (fire-and-forget)
...
> 🔴 **T-STUDIO sandbox (เฟส ก+ข):** `lib/train/sandbox.ts` `getTrainSandbox()`/`runInSandbox()`/`createSandbox()`/`trainUserId()`/`wrapSheetsForSandbox()`/`applyOverlayToTab()` — guard ใน **line.ts (reply/push/pushRaw*/loading/getProfile/download) · blob.ts · sheets/client.ts (proxy: ชีต Orders→fake grid · BotLibrary batchGet→ทับ draft overlay) · db.ts getSql() (→DATABASE_URL_TRAIN) · loader.ts (bypass cache 60วิ กัน draft รั่ว prod)** · 🔴 guard เช็ค ALS เท่านั้น — ไม่มี context = production เดิม · turn.ts `runTrainTurn`(+overlay)/`runTrainCron`/`runTrainReset`/`runTrainPreview` · **เฟส ข** preview.ts `buildReplySources`(provenance จาก verbatim log + re-run matcher)/`renderPreview`/`collectDroppedBubbles`(จาก var-guard log + **D-67** `image-dropped`) · lint.ts `lintPattern` · db: +`train_sessions` (2 DB · migrate อัตโนมัติต่อ connection)

**lib/config.ts** — `getConfig()` · `resolveFeatureSwitches(config) → FeatureSwitches` · `formatConfigForPrompt`
**lib/db.ts** — `ensureCustomer`/`mergePendingOrder`(items+order_id)/`reconcileWaitTags`(ลบเฉพาะ รอโอน/รอที่อยู่)/`resetCustomerMemory`(ล้าง human_mode+last_order · D-25/D-32)/**D-29** `isOrderWritten`/`markOrderWritten`(ตาราง `orders_written` · idempotency source of truth)/**D-32** `setLastOrder`(customers.last_order JSONB snapshot)/`setLastOrderLocked`(M=TRUE)/**D-45b** `addDeliveredStep`+`clearDeliveredStepsExceptCurrent`(`delivered_steps TEXT[]` · ธง "ส่งเนื้อหา step แล้ว" → เคยส่ง=ปิดท้ายอย่างเดียว · 🔴 **D-64: ล้างธงตอน `markOrderWritten` (เขียนชีตสำเร็จ)** ไม่ใช่ตอน cron · /reset ล้างหมด) ฯลฯ · CustomerState +`lastOrder`/`lastOrderLocked`/`deliveredSteps` · `CustomerState.tags` inject เข้า `<สถานะลูกค้า>` ทุกเทิร์น (คุมเงื่อนไขขายด้วยแท็กได้)

## 3. จุดประกอบ prompt (ตามลำดับ)
1. `line-webhook/handler.ts processMessage` (เดิมอยู่ route.ts · ย้ายตอน T-STUDIO ก) → `loadBotLibrary()` ได้ Steps/Knowledge/Products/Promo/Vars/Config
2. `buildStepInjection` + `buildFaqInjection` + `buildCatalogInjection` → stepText/faqText/catalogText · 🔴 **D-72a ลบ `buildObjectionInjection`+`objCap`** (v3 ยุบ Objections เข้า Knowledge ตั้งแต่ D-61.B → คืน text ว่างมาตลอด · prompt เหมือนเดิมทุก byte) · `buildExampleInjection` ลบตั้งแต่ v2.0 (D-41)
...
4. `runSalesTurn(...)` → **`prompt/system-v3.ts`** (D-68: ไฟล์เดียว · `buildSalesSystemV3` + `buildUserContent` ย้ายมาอยู่ที่นี่):
...
| `SHEET_BOTLIB_ID` | loader.ts (batchGet ทุกแท็บ) + train/write.ts — **คุม `salesCore`+`follow`** (🔴 D-72a คืนชื่อเดิม · **ค่าต้องเป็น id ของชีต v3** — เจ้าของแก้ค่าบน Vercel ก่อน deploy) |
...
| (ไม่มี env ใหม่ใน D-69 — โมเดล/thinking/timeout อยู่ในชีต CSV_Config) | |
...
| ~~`SHEET_SCHEMA`~~ | 🔴 **ลบแล้ว D-68** — โค้ดไม่อ่าน · เจ้าของลบออกจาก Vercel |
| ~~`SHEET_BOTLIB_V3_ID`~~ | 🔴 **ลบแล้ว D-72a** — คืนไปใช้ `SHEET_BOTLIB_ID` · เจ้าของลบ **หลัง** prod เขียว |
| `SHEET_BOTLIB_ID` | loader — ไฟล์ `SakbinBotLibrary-v3` (เจ้าของ own · แชร์ Editor ให้ SA) · 🔴 **D-72a: แท็บบนชีตต้องชื่อ Steps/Knowledge/Products/Promo/Vars/Config** → `lib/sheets/normalize-bundle.ts normalizeBundle` → BotLibrary (สถานะ "ว่าง=draft"→canonical ที่นี่) · seed: `scripts/seed-v3-sheet.ts` (idempotent) · mapping: docs/D61-MIGRATION.md |
...
**D-69 · การเรียก Gemini (ตั้งจากชีตได้ · ไม่มีแถว = ค่าเดิม ห้ามพัง):** `โมเดล`(alias `model`/`gemini_model` · default **gemini-3.5-flash**) · `ระดับการคิด`(alias `thinking`/`thinking_level` · default `low` — 🔴 3.x=enum minimal/low/medium/high · 2.x=ตัวเลข thinkingBudget · ส่งผิดตัว=400 → `resolveThinkingConfig` เลือกให้ + log `thinking-invalid` เมื่อค่าผิด) · `timeout_วินาที`(default **15** · เดิม 8 · clamp โดย `resolveGeminiTimeouts`: debounce+main+regen ≤ maxDuration−6 · regen ได้ครึ่งหนึ่ง) · `ข้อความ_ระบบช้า`(default ในโค้ด · `{ชื่อบอท}` ถูกแทนค่า)
`ชื่อบอท` · `ชื่อร้าน`(/`ชื่อร้าน/แบรนด์`) · `เพศบอท` · `ใช้ emoji`(/`ใช้_emoji`/`emoji`) · `temperature`(default **0.2** · D-44 เครื่องจำแนก · ชีตตั้งทับได้) · `maxOutputTokens`(พื้น 4096) · `แสดง_typing`(/`typing`) · `debounce_รวบคำถาม`(prefix `debounce`) · `หน่วง_ระหว่างบอลลูน`(/`หน่วง_ระหว่างข้อความ`) · `อายุลิงก์สลิป_วัน` · `เวลาตัดรอบออเดอร์`(/`เวลารอบตัดออเดอร์`) · ~~`เลขออเดอร์_รีเซ็ตทุกวัน`~~(ลบ D-64) · `คำ_handoff`(/`คำ_ส่งต่อแอดมิน`/`keyword_handoff`) · `คืนสิทธิ์บอท_หลังแชทเงียบ` · `ประโยคเปลี่ยนมือ_บอทรับต่อ` · `เปิด_คำสั่งเทสต์` · `โหมดประหยัดโควตา`(default **true** · 🔴 **D-61.C6: v3 บังคับ false ที่ `config.ts` ระดับโหมด** — จังหวะบอลลูนคือดีไซน์ CX + ซ้อม=จริง · v2 อ่านจากชีตเหมือนเดิม · เปิด=ยุบ `[[เว้น]]` ทุกตัวเป็น `\n\n` เหลือ message เดียว)
...
**intake (D-73c):** `ข้อความ_แจ้งแอดมิน_เก็บข้อมูล` (แจ้งตอนลูกค้าเข้าประตู `handoff_after_intake` · ไม่ปิดบอท · default ในโค้ด ห้ามมีคำว่า "รบกวน") · `เพดานเทิร์นก่อนส่งแอดมิน`(3) · `เทิร์นขั้นต่ำก่อนส่งแอดมิน`(1)
**แก้/ยกเลิกออเดอร์ (D-77):** `ข้อความ_แจ้งแอดมิน_แก้ออเดอร์` (บรรทัดท้าย 🔔 แก้ออเดอร์ · default ไม่มี "รบกวน") · `คำ_ยกเลิกออเดอร์` (พยานชั้น 2 ของเส้นยกเลิก · default ยกเลิก,ไม่เอาแล้ว,ไม่สั่งแล้ว,ขอเลิก)
**price guard (D-27 · KI-02):** `โหมดราคาผิด`(`เตือน`=default/`บล็อก`)
**ทักทายรายวัน (D-51 · delivery ล้วน):** `ทักทายรายวัน` — เติมหน้าบอลลูนข้อความแรก เทิร์นแรกของลูกค้าในแต่ละวัน (เวลาไทย) · ไม่มี key = ค่าเริ่ม `"สวัสดีค่ะ "` · ว่าง `""` = ปิด · ข้าม handoff/degraded/รูป-fallback · `lib/greeting.ts` (`isFirstMessageOfDay`/`prependToFirstTextBubble`)
**แจ้งเลขพัสดุ (D-50 · ก้อน B · cron):** `ข้อความแจ้งพัสดุ` (template · `{ขนส่ง}{เลขพัสดุ}` · ว่าง=ปิด · ไม่มี key=ค่าเริ่ม) · `ขนส่ง_เริ่มต้น` (default `Shopee Express`) · `lib/shipping.ts` · cron: P(Tracking)ไม่ว่าง+O=TRUE → push ลูกค้า (dedup Neon `shipping_notified`) ผ่าน greeting D-51 · human_mode→แจ้งแอดมิน
**คิดเอง / verbatim (P2 · ชั้น③ · คอลัมน์ในแถว ไม่ใช่ CSV_Config):** คอลัมน์ `คิดเอง` (optional) ใน **Steps** — `ปิด`=ส่ง pattern เป๊ะ (แทนตัวแปรอย่างเดียว ไม่ให้ AI แต่ง) · ว่าง/`เปิด`=AI เรียบเรียงเดิม
...
~~**objections (D-27):** `จำนวนข้อโต้แย้งที่ยัดเข้า prompt`~~ (🔴 **ลบ D-72a** — โค้ดไม่อ่านคีย์นี้แล้ว) · ~~`จำนวนตัวอย่างที่ยัดเข้า prompt`~~ (ลบ v2.0 D-41 · เลิก Examples)
**order_id (D-29):** `รหัสนำหน้าออเดอร์`(default `SKB`) · 🔴 **D-64: เลขลำดับ (col A) = Apps Script บนชีต** รูปแบบ `MMDD_n` — โค้ดไม่เขียน/ไม่แจกแล้ว
...
**v3 (D-61.A · D-68: เป็นทางเดียวแล้ว):** `คำ_ธงสุขภาพ`(ชื่อเดียวไม่มี alias · ไม่มี key=default ตาข่ายในโค้ด `DEFAULT_HEALTH_FLAG_KEYWORDS` · ว่าง=ปิด) · `คำรับรอง_ต้องห้าม`(list assurance guard · default ในโค้ด) · `ข้อความ_แจ้งแอดมิน_notify`(เนื้อ 🔔 ธงสุขภาพ) — โค้ดใหม่: `lib/schema-mode.ts` · `prompt/system-v3.ts` · `lib/guards/assurance.ts` · handler v3 (matchHealthFlagV3 · composeReplyV3 · applyAssuranceGuardV3 · pushHealthNotifyV3 — 🔴 D-68 ลบคู่ V2 ทุกตัว) · marker `__HEALTH_NOTIFY__` ใน delivered_steps · **D-62:** `findAssuranceHits` มี allowlist แคบ — "เชิงระวัง `เพื่อความ...<คำ>` (ไม่มีช่องว่างคั่น) + บรรทัดเดียวกันมีวลีส่งต่อแพทย์/ทีม" = ไม่ hit · regen มี hit ใดๆ → ทิ้ง ใช้ต้นฉบับ-ตัดบรรทัด + log `regen-dirty`
...
|---|---|---|
| ข้อเท็จจริง | Knowledge · Products · Promo | **ท่อง** copy ตรง ห้ามเรียบเรียง |
| หลักการ | Steps (~~CSV_Objections~~ ลบ D-72a) | **เข้าใจ** ประกอบคำตอบเอง |
| น้ำเสียง | ~~CSV_Examples~~ (ลบ D-41) | **เลียนสไตล์** ห้ามลอกถ้อยคำ |
| กฎเหล็ก | System Prompt (`prompt/system-v3.ts`) | **ห้ามฝ่าฝืน** ไม่ว่าเข้าใจอะไรมา |
